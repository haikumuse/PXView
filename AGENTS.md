...
- **Typed event bus (fix-all-architecture-issues Task 3):** new code MUST use `IEventListener` + `broadcast<T>()` (defined in `interface/events.h`) instead of raw `broadcast_msg(DSV_MSG_*)`. 18 typed event structs carry full context. `broadcast<T>()` has a `thread_local _broadcast_depth` loop guard (depth>1 → assert + log + short-circuit). `OnMessage` translates 15 notification DSV_MSG_* to typed events for backward compat. `MainWindow` still uses `IMessageListener` (not yet migrated).
