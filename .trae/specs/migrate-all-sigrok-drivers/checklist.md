...
> **更新（2026-07-02，tiered-driver-compat-fix spec 完成）**：libsigrok 层全量编译通过。以下驱动的"驱动编译通过"项已在 tiered-driver-compat-fix spec 中验证（标记 [x]）：
> - Batch 1：saleae-logic-pro、asix-sigma、kingst-la2016、openbench-logic-sniffer、lecroy-logicstudio、pipistrello-ols、sysclk-lwla
> - Batch 2：rigol-ds、siglent-sds、lecroy-xstream、uni-t-ut181a、rigol-dg
> - Batch 3：fluke-45、agilent-dmm、testo
>
> "出现在设备列表"项需运行时验证（PXView 应用层有独立的 OnMessage 签名问题待修复，不在 driver spec 范围）。
...
- [ ] `zeroplus-logic-cube` 驱动出现在设备列表
- [x] `sysclk-lwla` 驱动编译通过
- [ ] `sysclk-lwla` 驱动出现在设备列表
- [ ] `sysclk-sla5032` 驱动编译通过
- [ ] `sysclk-sla5032` 驱动出现在设备列表
- [ ] `ftdi-la` 驱动编译通过
- [ ] `ftdi-la` 驱动出现在设备列表
- [x] `lecroy-logicstudio` 驱动编译通过
...
- [x] `rigol-ds` 驱动编译通过（SCPI 依赖）
- [ ] `rigol-ds` 驱动出现在设备列表
- [x] `siglent-sds` 驱动编译通过（SCPI 依赖）
...
- [x] `lecroy-xstream` 驱动编译通过（SCPI 依赖）
