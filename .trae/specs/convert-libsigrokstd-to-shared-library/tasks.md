...
- [x] Task 1: 创建跨平台导出宏头文件 `libsigrokstd/include/srstd_export.h`
...
- [x] Task 2: 修改 [libsigrokstd/CMakeLists.txt](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/libsigrokstd/CMakeLists.txt) 切换为 SHARED 库
...
- [x] Task 5: 修改 [libsigrokstd/bridge/srstd_pxview_glue.h](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/libsigrokstd/bridge/srstd_pxview_glue.h) 添加 SRSTD_API
  - `#include "srstd_export.h"`(在 glib.h 之后)
  - 所有公开函数声明前加 `SRSTD_API` 前缀(共约 25 个函数)
  - 保持函数签名不变(参数仍用 `void*`)
- [x] Task 6: 修改 [libsigrokstd/bridge/srstd_pxview_glue.c](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/libsigrokstd/bridge/srstd_pxview_glue.c) 内部调用真实 sr_*
...
- [x] Task 7: 修改 [libsigrokstd/bridge/srstd_init_shared.c](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/libsigrokstd/bridge/srstd_init_shared.c) 同 Task 6
  - 内部调用真实 `sr_init` / `sr_exit` / `libusb_*`
  - struct `struct sr_context*`(上游真实类型)
- [x] Task 8: 修改 [libsigrokstd/bridge/srstd_bridge.{h,c}](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/libsigrokstd/bridge/srstd_bridge.h) 类型引用真实化
...
- [x] Task 9: 修改 [libsigrokstd/src/driver_registry.c](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/libsigrokstd/src/driver_registry.c)(如有 #include "srstd.h" 改为 #include <libsigrok/libsigrok.h>)
  - 验证:82 个驱动的 `<driver>_driver_info` 符号仍被手动注册
  - 实际:driver_registry.c 已在 Phase 2 删除,改用上游 section 机制(driver_list_start.c/driver_list_stop.c + HAVE_DRIVERS),81 个驱动通过 `__sr_driver_list` section 注册

## Phase 4: 接入 PXView(依赖 Phase 3)

- [x] Task 10: 修改顶层 [CMakeLists.txt](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/CMakeLists.txt) 添加 libsigrokstd 子项目
...
- [x] Task 11: 修改 [PXView/pv/sigsession.h](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/PXView/pv/sigsession.h) 添加 srstd 上下文成员
  - 在 private 区新增 `void* _srstd_ctx = nullptr;`
  - 新增 `void* _libusb_ctx_cache = nullptr;`(供 init_shared 使用,避免重复查询)
- [x] Task 12: 修改 [PXView/pv/sigsession.cpp](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/PXView/pv/sigsession.cpp) init/uninit 调用 srstd
...
- [x] Task 13: 修改 [PXView/pv/deviceagent.h](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/PXView/pv/deviceagent.h) + `.cpp` 添加 LIB_SRSTD 分流
  ...
  - `get_config`/`set_config`/`get_config_list`/`start`/`stop`/`is_collecting` 方法添加 if 分流:
    ...
    // 原 PXView 路径
    ```
- [x] Task 14: 修改 [PXView/pv/sigsession.cpp](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/PXView/pv/sigsession.cpp) `get_device_list()` 合并扫描结果
  ...
  - 合并到返回列表
- [x] Task 15: 修改 [PXView/pv/sigsession.cpp](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/PXView/pv/sigsession.cpp) `set_device()` 分流
...
- [x] Task 16: 修改 [cmake/install_packaging.cmake](file:///c:/Users/admin/Downloads/DSView-main_2026_4_27cppnb/cmake/install_packaging.cmake) 安装动态库
  ...
  - 实际:Windows 已验证 `install.dir/bin/liblibsigrokstd.dll` 安装成功;Linux/macOS RPATH 未配置(本机为 Windows 环境)
- [x] Task 17: 修正前序 spec 勾选状态
  ...
  - 不删除该 spec 文档
  - 实际:已标注 Phase 3 全部 17 个条目 + 添加节首说明注解
- [x] Task 18: 编译验证(部分通过)
  - `ninja libsigrokstd` 成功(exit 0,无 work to do — 已构建)
  - `ninja pxview-core` 成功(exit 0,无 work to do — 已构建)
  - `cmake --install .` 成功(exit 0)— `install.dir/bin/liblibsigrokstd.dll` + `install.dir/lib/liblibsigrokstd.dll.a` 均已安装
  - `install.dir/bin/PXView.exe` 存在(旧版本,因预存在链接问题无法重新构建)
  - **预存在问题**:`ninja install`(完整构建+安装)失败,因 `PXView/pv/mainwindow.cpp` 使用 `view::ViewStatus`/`view::Viewport` 的前向声明(incomplete type)导致编译错误 — 此问题与本次 spec 改动无关(view.h 前向声明重构遗留,git stash 验证确认)
  - 启动日志验证("libsigrokstd initialized")延期 — 需先修复 PXView.exe 链接问题
- [x] Task 19: 运行时验证(受限 — 仅 DLL 导出符号验证)
  - 启动 PXView.exe --headless 延期(需先修复 PXView.exe 链接问题)
  - 已验证 DLL 导出符号(用 objdump -p):
    - 总导出符号:2290 个(与 spec 一致)
    - `sr_driver_list__start` / `sr_driver_list__stop` 存在(section 边界,81 驱动通过 `__sr_driver_list` section 注册)
    - `sr_driver_list` 存在(sr_drivers_init 收集 section 驱动)
    - 20 个 `srstd_glue_*` 符号(胶水层 API:scan/open/config/trigger/acquisition 全套)
    - `srstd_init_shared` / `srstd_pxview_init_shared` / `srstd_pxview_exit` 存在(初始化/退出包装)
    - 20+ 个 `srstd_bridge_*` 符号(数据结构转换层)
    - `sr_init` / `sr_exit` / `sr_config_get` / `sr_dev_open` / `sr_session_new` / `sr_session_destroy` 存在(上游 SR_API)
  - MCP get_devices 验证延期(需先修复 PXView.exe 链接问题)
