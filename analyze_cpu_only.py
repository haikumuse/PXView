...
wait_funcs = ["NtWaitForSingleObject", "NtUserMsgWaitForMultipleObjectsEx", "NtWaitForMultipleObjects", "NtAlpcSendWaitReceivePort", "NtUserGetMessage", "NtUserPeekMessage"]

threads = data.get('threads', [])
for t in threads:
    name = t.get('name') or f"TID-{t.get('tid')}"
    ...
    for stack_idx, time_val, weight in zip(stack_list, time_list, weight_list):
        ...
        if time_val >= start_window:
            ...
            if not call_stack:
                continue
            leaf = call_stack[-1]
            if not any(w in leaf for w in wait_funcs):
                in_window_exclusive[leaf] += weight
                in_window_weight += weight
            
    if in_window_weight > 50:
        print(f"\n=======================================================")
        print(f"THREAD: {name} (TID: {t.get('tid')}) - ACTIVE CPU Samples >= 6.7s: {in_window_weight}")
        print(f"=======================================================")
        
        print("TOP 5 NON-WAIT FUNCTIONS:")
        sorted_in = sorted(in_window_exclusive.items(), key=lambda x: x[1], reverse=True)
        for func, cnt in sorted_in[:10]:
