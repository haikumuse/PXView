...
$FoldersToCopy = @("PXView", "libsigrok", "libsigrokdecode", "common", "lang", "CMake", "debian")
$FilesToCopy = @(
...
if (Test-Path "$SourceDir\oridoc") {
    Write-Host "Copying and mapping folder: oridoc -> doc" -ForegroundColor Yellow
    if (-Not (Test-Path "$TargetDir\doc")) {
        New-Item -ItemType Directory -Force -Path "$TargetDir\doc" | Out-Null
    }
    Copy-Item -Path "$SourceDir\oridoc\*" -Destination "$TargetDir\doc" -Recurse -Force
}
