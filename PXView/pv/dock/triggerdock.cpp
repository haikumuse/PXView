...
TriggerDock::TriggerDock(QWidget *parent, SigSession *session)
    : pv::widgets::SmoothScrollArea(parent), _session(session), _data_src(session),
      _context(nullptr) {
  _cur_ch_num = 16;
  if (_data_src->device()->have_instance()) {
    _data_src->device()->get_config_int16(SR_CONF_TOTAL_CH_NUM, _cur_ch_num);
  }
...
void TriggerDock::adv_trigger() {
  if (_data_src->device()->is_hardware_logic()) {
    // SR_CONF_STREAM fork key deleted — use DeviceAgent typed wrapper.
    bool stream = _data_src->device()->is_stream_mode();
  ...
  } else if (_data_src->device()->is_file() == false) {
...
void TriggerDock::device_updated() {
  ...
  if (mode == LOGIC) {
    ...
    stream = _data_src->device()->is_stream_mode();
    ...
    if (_data_src->device()->is_virtual() || stream) {
      _simple_radioButton->setChecked(true);
      simple_trigger();
    }
  }

  bool ret = _data_src->device()->get_config_int16(SR_CONF_TOTAL_CH_NUM, ch_num);
  if (ret) {
...
QJsonObject TriggerDock::get_session() {
  ...
  const auto &cfg = _data_src->trigger_config();
  QJsonObject trigSes;
...
void TriggerDock::refresh_ui_from_core() {
  ...
  const auto &tcfg = _data_src->trigger_config();
...
void TriggerDock::try_commit_trigger() {
  ...
  int mode = _data_src->device()->get_work_mode();
  bool bInstant = _data_src->is_instant();
  ...
  if (commit_trigger() == false) {
    ...
    if (has_view) {
      ...
      auto &sigs = _data_src->get_signal_models();
      for (auto s : sigs) {
