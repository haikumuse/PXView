...
void SearchDock::bind_context(TabContext *ctx) {
  ...
  _result_model->clear();
  _result_model->set_samplerate(_data_src->cur_snap_samplerate());
...
void SearchDock::search_worker() {
  const auto snapshot = _data_src->get_snapshot(SR_CHANNEL_LOGIC);
  if (!snapshot)
    return;
...
int64_t SearchDock::parse_time_text(const QString &text, bool &is_row_index) {
  ...
  if (_session)
    samplerate = _data_src->cur_snap_samplerate();
