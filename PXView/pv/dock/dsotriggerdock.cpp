...
DsoTriggerDock::DsoTriggerDock(QWidget *parent, SigSession *session)
    : pv::widgets::SmoothScrollArea(parent), _session(session), _data_src(session), _context(nullptr) {
...
void DsoTriggerDock::pos_changed(int pos) {
  ...
  ret = _data_src->device()->set_config_double(SR_CONF_HORIZ_TRIGGERPOS, (double)pos);
  if (!ret) {
    if (_data_src->device()->is_hardware() || true) {
...
void DsoTriggerDock::hold_changed(int hold) {
  ...
  ret = _data_src->device()->set_config_uint64(SR_CONF_TRIGGER_HOLDOFF,
                                                  holdoff);
...
void DsoTriggerDock::margin_changed(int margin) {
  int ret;
  ret = _data_src->device()->set_config_byte(SR_CONF_TRIGGER_MARGIN, margin);
  if (!ret) {
...
void DsoTriggerDock::source_changed() {
  ...
  ret = _data_src->device()->set_config_byte(SR_CONF_TRIGGER_SOURCE, id);
  if (!ret) {
...
void DsoTriggerDock::check_setting() {
  if (check_trig_channel() == false) {
    ...
    _data_src->device()->set_config_byte(SR_CONF_TRIGGER_SOURCE,
...
bool DsoTriggerDock::check_trig_channel() {
  int id = _source_group->checkedId();
  bool b0 = _data_src->device()->channel_is_enable(0);
  bool b1 = _data_src->device()->channel_is_enable(1);
...
void DsoTriggerDock::channel_changed(int ch) {
  ...
  ret = _data_src->device()->set_config_byte(
...
void DsoTriggerDock::type_changed() {
  ...
  ret = _data_src->device()->set_config_byte(SR_CONF_TRIGGER_SLOPE, id);
  if (!ret) {
...
void DsoTriggerDock::device_change() {
  bool bDisable = _data_src->device()->is_file();
...
void DsoTriggerDock::update_view() {
  ...
  double dpos;
  if (_data_src->device()->get_config_double(SR_CONF_HORIZ_TRIGGERPOS, dpos)) {
    _position_slider->setValue((int)dpos);
  }

  if (_data_src->device()->get_config_byte(SR_CONF_TRIGGER_SOURCE, src)) {
  ...
  ret = _data_src->device()->get_config_byte(SR_CONF_TRIGGER_CHANNEL, src);
  if (ret) {
  ...
  ret = _data_src->device()->get_config_byte(SR_CONF_TRIGGER_SLOPE, slope);
  if (ret) {
  ...
  uint64_t holdoff;
  ret = _data_src->device()->get_config_uint64(SR_CONF_TRIGGER_HOLDOFF,
                                                  holdoff);
  if (ret) {
    auto v = holdoff * 10.0;
  ...
  int margin;
  ret = _data_src->device()->get_config_byte(SR_CONF_TRIGGER_MARGIN, margin);
  if (ret) {
...
void DsoTriggerDock::set_session(QJsonObject &obj) {
  ...
  if (obj.contains("triggerSource")) {
    ...
    if (btn)
      btn->setChecked(true);
    _data_src->device()->set_config_byte(SR_CONF_TRIGGER_SOURCE, src);
  }
  if (obj.contains("triggerSlope")) {
    ...
    if (btn)
      btn->setChecked(true);
    _data_src->device()->set_config_byte(SR_CONF_TRIGGER_SLOPE, slope);
  }
