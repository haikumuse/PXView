    ...
    , _lastFrameTime(0)
{
    ...
    connect(&(_->animationHideBar), &QPropertyAnimation::valueChanged, this, [this](const QVariant &value){
        qint64 now = _frameTimer.elapsed();
        qint64 delta = now - _lastFrameTime;
        _lastFrameTime = now;
        dsv_info("QRibbon frame: minH=%d delta=%lldms", value.toInt(), delta);
    });
...
{
    dsv_info("QRibbon::uninstall TBD...");
}
