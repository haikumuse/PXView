...
wait_funcs = ["NtWait", "NtUserMsgWait", "NtDelayExecution", "NtAlpcSendWaitReceivePort", "NtUserGetMessage", "NtUserPeekMessage", "Sleep", "WaitFor"]
...
for t in threads:
    ...
    for stack_idx, time_val, weight in zip(stack_list, time_list, weight_list):
        ...
        if time_val >= start_window:
            ...
            if not call_stack:
                continue
            leaf = call_stack[-1]
            is_wait = any(w.lower() in leaf.lower() for w in wait_funcs)
            if not is_wait:
    ...
    if in_window_weight > 50:
        print(f"\n=======================================================")
        print(f"THREAD: {name} (TID: {t.get('tid')}) - TRUE ACTIVE CPU Samples >= 6.7s: {in_window_weight}")
        print(f"=======================================================")
        
        sorted_in = sorted(in_window_exclusive.items(), key=lambda x: x[1], reverse=True)
        for func, cnt in sorted_in[:5]:
