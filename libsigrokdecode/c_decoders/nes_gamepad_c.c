    ...
    const char *cmd, const unsigned char *data, uint64_t data_len)
{
    ...
    if (miso == 0xFF) {
        C_ANN_PUT(di, start_sample, end_sample, s->out_ann, ANN_NO_PRESS,
                  "No button is pressed");
    } else if (miso == 0x00) {
        C_ANN_PUT(di, start_sample, end_sample, s->out_ann, ANN_NOT_CONNECTED,
...
{
    nes_gamepad_state *s = (nes_gamepad_state *)c_decoder_get_private(di);
    s->out_ann = c_decoder_register_output(di, SRD_OUTPUT_ANN, "nes_gamepad");

    const char *variant = c_decoder_get_option_string(di, "variant", "Standard gamepad");
