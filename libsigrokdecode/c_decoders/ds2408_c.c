    ...
    const char *cmd, const unsigned char *data, uint64_t data_len)
{
    ...
    if (strcmp(cmd, "ROM") == 0) {
        ...
        uint8_t family_code = data_len > 0 ? data[0] : 0;
        char buf[128];
    ...
    if (s->state == STATE_CMD) {
        ...
        if (s->num_bytes == 2) {
            ...
            C_ANN_PUT(di, start_sample, end_sample, s->out_ann, ANN_PIO, buf);
        }
        ...
        if (val == 0xaa) {
            C_ANN_PUT(di, start_sample, end_sample, s->out_ann, ANN_STATUS, "Success");
        } else if (val == 0xff) {
            C_ANN_PUT(di, start_sample, end_sample, s->out_ann, ANN_STATUS, "Fail New State");
        } else {
            char buf[32];
            snprintf(buf, sizeof(buf), "Ack: 0x%02x", val);
            C_ANN_PUT(di, start_sample, end_sample, s->out_ann, ANN_STATUS, buf);
        }
        s->state = STATE_IDLE;
        ...
        if (s->num_bytes == 2) {
            ...
            if (val == 0xaa) {
                ...
                C_ANN_PUT(di, start_sample, end_sample, s->out_ann, ANN_WARN, "Invalid byte");
            }
        }
    }
...
{
    ds2408_state *s = (ds2408_state *)c_decoder_get_private(di);
    s->out_ann = c_decoder_register_output(di, SRD_OUTPUT_ANN, "ds2408");
}
