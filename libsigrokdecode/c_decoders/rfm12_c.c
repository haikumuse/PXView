...
static void rfm12_handle_configuration_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_PARAMS, buf, frequencies[freq_idx]);
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_PARAMS, buf, cap_buf);
...
static void rfm12_handle_power_management_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Power management", "Power");
...
static void rfm12_handle_frequency_setting_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Frequency setting", "Frequency");
    ...
    if (s->num_bytes >= 3) {
        ...
        if (s->last_freq != (uint16_t)f)
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION, "Changing", "~");
...
static void rfm12_handle_data_rate_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Data rate command", "Data rate");
...
static void rfm12_handle_receiver_control_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Receiver control command");
...
static void rfm12_handle_data_filter_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Data filter command");
...
static void rfm12_handle_fifo_and_reset_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "FIFO and reset command");
...
static void rfm12_handle_synchron_pattern_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Synchron pattern command");
...
static void rfm12_handle_fifo_read_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "FIFO read command", "FIFO read");
    ...
    if (s->num_bytes >= 2) {
        ...
        C_ANN_PUT(di, ss, es, s->out_ann, ANN_RETURN, long_str, short_str);
    }
}
...
static void rfm12_handle_afc_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "AFC command");
...
static void rfm12_handle_transceiver_control_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Transceiver control command");
...
static void rfm12_handle_pll_setting_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "PLL setting command");

    C_ANN_PUT(di, ss, es, s->out_ann, ANN_PARAMS, "Clock buffer rise and fall time");
...
static void rfm12_handle_transmitter_register_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Transmitter register command", "Transmit");
...
static void rfm12_handle_software_reset_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    uint64_t ss, es;
    rfm12_get_span(s, &ss, &es);
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Software reset command");
}
...
static void rfm12_handle_wake_up_timer_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Wake-up timer command", "Timer");
...
static void rfm12_handle_low_duty_cycle_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    uint64_t ss, es;
    rfm12_get_span(s, &ss, &es);
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Low duty cycle command");
}
...
static void rfm12_handle_low_battery_detector_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Low battery detector command");
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_PARAMS, buf, clock_names[clock_idx]);

    double v = 2.25 + (cmd1 & 0x0F) * 0.1;
    char v_buf[16];
    snprintf(v_buf, sizeof(v_buf), "%.2fV", v);
    snprintf(buf, sizeof(buf), "Low battery voltage: %s", v_buf);
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_PARAMS, buf, v_buf);
}
...
static void rfm12_handle_status_read_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, "Status read command", "Status");
    ...
    if (s->num_bytes >= 2) {
        ...
        if (ret0 & 0x80) {
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION,
                      receiver_enabled ? "Received data in FIFO" : "Transmit register ready");
        }
        if (ret0 & 0x40)
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION, "Power on Reset");
        if (ret0 & 0x20) {
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION,
                      receiver_enabled ? "RX FIFO overflow" : "Transmit register under run");
        }
        if (ret0 & 0x10)
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION, "Wake-up timer");
        if (ret0 & 0x08)
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION, "External interrupt");
        if (ret0 & 0x04)
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION, "Low battery");
        if (ret0 & 0x02)
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION, "FIFO is empty");
        if (ret0 & 0x01) {
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION,
                      receiver_enabled ? "Incoming signal above limit" : "Antenna detected RF signal");
        }
        if (ret1 & 0x80)
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION, "Data quality detector");
        if (ret1 & 0x40)
            C_ANN_PUT(di, ss, es, s->out_ann, ANN_INTERPRETATION, "Clock recovery locked");

        /* AFC offset */
        C_ANN_PUT(di, ss, es, s->out_ann, ANN_RETURN, "AFC offset");
...
static void rfm12_handle_cmd(struct srd_decoder_inst *di, rfm12_state *s)
{
    ...
    if (cmd0 == 0x80)
        ...
        C_ANN_PUT(di, ss, es, s->out_ann, ANN_CMD, buf);
    }
}
...
{
    rfm12_state *s = (rfm12_state *)c_decoder_get_private(di);
    s->out_ann = c_decoder_register_output(di, SRD_OUTPUT_ANN, "rfm12");
}
