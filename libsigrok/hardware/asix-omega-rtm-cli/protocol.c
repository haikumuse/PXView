 ...
 * implementations here match the microchip-pickit2 / rdtech-tc pattern.
 * ------------------------------------------------------------------------- */
 ...
 * functions that standard sigrok exposes via libsigrok-internal.h. The
 * only PXView driver that references them (kingst-la2016) is OFF by
 * default and would fail to link if enabled. This driver therefore
 * provides its own minimal local implementation covering the four entry
 * points that the RLE decompression path uses:
 *
 ...
 * from the 16-bit timestamp). flush() sends whatever is pending as a
 * single SR_DF_LOGIC packet via sr_session_send() (which the compat
 * layer maps to PXView's ds_data_forward()).
 *
 ...
 * but matches what fx2lafw, asix-sigma and session_driver.c all do.
 * ========================================================================== */
...
static struct feed_queue_logic *feed_queue_logic_alloc(
	const struct sr_dev_inst *sdi,
	size_t sample_count, size_t unit_size)
{
	...
	q->count_samples = 0;
	q->data = g_malloc(sample_count * unit_size);
	return q;
}

static int feed_queue_logic_flush(struct feed_queue_logic *q)
{
	...
	logic.length = q->count_samples * q->unit_size;
	logic.unitsize = q->unit_size;
	logic.data = q->data;
...
static int feed_queue_logic_submit_one(struct feed_queue_logic *q,
	const uint8_t *data, size_t repeat_count)
{
	...
	while (repeat_count > 0) {
		...
		if (avail == 0) {
			...
			feed_queue_logic_flush(q);
			avail = q->cap_samples;
		}
		to_copy = (repeat_count < avail) ? repeat_count : avail;
		while (to_copy > 0) {
			memcpy(q->data + q->count_samples * q->unit_size,
				data, q->unit_size);
			q->count_samples++;
			to_copy--;
			repeat_count--;
...
static void feed_queue_logic_free(struct feed_queue_logic *q)
{
	if (!q)
		return;
...
/* ===========================================================================
 * Driver protocol implementation. Closely follows the upstream sigrok
 * source, with only the PXView-specific adjustments:
 *   - the receive callback takes (const struct sr_dev_inst *sdi) directly
 *     instead of (void *cb_data)
 *   - feed_queue_logic_* calls resolve to the local static helpers above
 *   - sr_hexdump_new/free resolve to the local helpers via protocol.h
 *     macros
 * ========================================================================== */
		...
		const struct sr_dev_inst *sdi)
{
	...
	if (revents & G_IO_ERR) {
		...
		(void)sr_dev_acquisition_stop(sdi);
	}
