 ...
 */
static int send_request(struct sr_tcp_dev_inst *tcp,
	const uint8_t *data, size_t dlen)
{
	...
	ret = sr_tcp_write_bytes(tcp, data, dlen);
	if (ret < 0)
		return ret;
	written = (size_t)ret;
	if (written != dlen)
 ...
 */
static int recv_response(struct sr_tcp_dev_inst *tcp,
	uint8_t *data, size_t dlen)
{
	int ret;
	size_t got;

	ret = sr_tcp_read_bytes(tcp, data, dlen, FALSE);
	if (ret < 0)
		return ret;
	got = (size_t)ret;
	if (sr_log_loglevel_get() >= SR_LOG_SPEW) {
...
static int send_then_recv(struct sr_tcp_dev_inst *tcp,
	...
	uint8_t *rx_data, size_t rx_length)
{
	...
	if (tx_data && tx_length) {
		ret = send_request(tcp, tx_data, tx_length);
		if (ret != SR_OK)
			return ret;
	}

	if (rx_data && rx_length) {
		ret = recv_response(tcp, rx_data, rx_length);
		if (ret != SR_OK)
			return ret;
	}
	...
	uint8_t *model_code, uint8_t *hw_version, uint8_t *fw_version)
{
	...
	ret = send_then_recv(tcp, req, wrptr - req, rsp, sizeof(rsp));
	if (ret != SR_OK)
		return ret;
	rdptr = rsp;
...
SR_PRIV int devantech_eth008_cache_state(const struct sr_dev_inst *sdi)
{
	...
	if (devc->model->ch_count_do) {
		...
		ret = send_then_recv(tcp, req, wrptr - req, rsp, rx_size);
		if (ret != SR_OK)
			return ret;
		rdptr = rsp;
