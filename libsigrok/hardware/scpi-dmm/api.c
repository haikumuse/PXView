 ...
 * hwdriver.c's drivers_list[] entry `&scpi_dmm_driver_info` can resolve
 * at link time. The actual definition is at the end of this file.
 * (Matches the hp-3457a/hp-3478a pattern; the original sigrok source used
 ...
 * 34405A and 34466A. It differs in starting the measurement with INIT: using
...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	 ...
	 * pattern as scpi-pps/api.c, motech-lps-30x/api.c, hp-3457a/api.c
	 * and hp-3478a/api.c).
	 */
	conn = NULL;
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_DEVICE_OPTIONS:
		/*
		 * Without a device context, fall back to the generic device
		 * options array. With a device context, expose the per-model
		 * device options. PXView's std_config_list() takes the same
		 * arguments as standard sigrok's std_opts_config_list(), so
		 * call it directly with the model's devopts/devopts_size.
		 */
		if (!devc)
 ...
 * scpi_dmm_receive_data now has PXView's const-sdi signature, so the
 * (void *)sdi cast matches the void *cb_data parameter of sr_scpi_source_add.
 *
 * std_session_send_df_header() in PXView's compat layer is the 2-arg form
 * (sdi, prefix); matching the hp-3457a / hp-3478a compat drivers.
 */
 ...
 *
 * Note: The original scpi-dmm driver does not provide a config_channel_set
...
static struct sr_dev_driver *scpi_dmm_drv_ptr;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int scpi_dmm_compat_init(struct sr_context *sr_ctx)
{
	scpi_dmm_drv_ptr = &scpi_dmm_driver_info;
	return std_init(scpi_dmm_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int scpi_dmm_compat_cleanup(void)
{
	/* Clear device instances before tearing down the driver context. */
	std_dev_clear(scpi_dmm_drv_ptr);
	return std_cleanup(scpi_dmm_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *scpi_dmm_compat_scan(GSList *options)
{
	return scan(scpi_dmm_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int scpi_dmm_compat_config_get(int id, GVariant **data,
...
static int scpi_dmm_compat_config_set(int id, GVariant *data,
	struct sr_dev_inst *sdi, struct sr_channel *ch,
	struct sr_channel_group *cg)
{
	/*
	 * The original scpi-dmm driver does not provide a config_channel_set
...
static int scpi_dmm_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int scpi_dmm_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
static int scpi_dmm_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver scpi_dmm_driver_info = {
	.name = "scpi-dmm",
	.longname = "SCPI DMM",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = scpi_dmm_compat_init,
	.cleanup = scpi_dmm_compat_cleanup,
	.scan = scpi_dmm_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = scpi_dmm_compat_config_get,
	.config_set = scpi_dmm_compat_config_set,
	.config_list = scpi_dmm_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = scpi_dmm_compat_acquisition_start,
	.dev_acquisition_stop = scpi_dmm_compat_acquisition_stop,
	.priv = NULL,
};
