 ...
 * sr_atod_ascii_digits(), which PXView's libsigrok does not provide. Parse
 * ASCII numeric strings in a locale-independent way and return SR_OK/SR_ERR.
 * These are kept file-local (static) because they are only used within this
 * translation unit (same approach as agilent-dmm/protocol.c).
 */
static int sr_atoi_local(const char *str, int *ret)
{
	char *e;
	long tmp;
...
static int sr_atod_ascii_local(const char *str, double *ret)
{
	char *e;
	double tmp;
...
/*
 * Like sr_atod_ascii(), but also extracts the exponent value (the "E<exp>"
 * suffix) into *digits. Mirrors standard sigrok's sr_atod_ascii_digits()
 * helper that the original scpi-dmm driver used to read range text along
 * with its precision exponent.
 */
static int sr_atod_ascii_digits_local(const char *str, double *ret, int *digits)
{
	char *e;
	double tmp;
	const char *p;
	...
	*ret = tmp;
	...
	/* Skip any trailing exponent suffix like "E-3" or "e+1". */
	p = e;
	...
	if (p && *p && (*p == 'e' || *p == 'E')) {
		...
		errno = 0;
		{
			long exp_val = strtol(p, &e, 10);
			if (e == p || errno != 0)
				return SR_ERR;
			if (exp_val > INT_MAX || exp_val < INT_MIN)
				return SR_ERR;
			*digits = (int)exp_val;
		}
	}
...
/* Map the standard sigrok helper names to the file-local implementations. */
#define sr_atoi(str, ret)            sr_atoi_local((str), (ret))
#define sr_atod_ascii(str, ret)      sr_atod_ascii_local((str), (ret))
#define sr_atod_ascii_digits(str, ret, digits) \
	sr_atod_ascii_digits_local((str), (ret), (digits))
...
/*
 * PXView note: the original sigrok source used sr_analog_init() to wire up
 * an analog struct's encoding/meaning/spec sub-struct pointers, then wrote
 * fields like analog->encoding->digits, analog->meaning->mq, etc. PXView's
 * sr_datafeed_analog is the old flat layout (probes, num_samples, unit_bits,
 * unit_pitch, mq, unit, mqflags, data) -- there are no encoding/meaning/spec
 * sub-structs and no sr_analog_init() helper. Fields are written directly
 * here (analog->mq, analog->mqflags, analog->unit, analog->data,
 * analog->unit_bits). The 'digits' calculation is kept to preserve the
 * original driver's precision logic but the result is unused (PXView's flat
 * analog struct has no digits field), matching the hp-3457a / hp-3478a
 * compat driver pattern.
 */
SR_PRIV int scpi_dmm_get_meas_agilent(const struct sr_dev_inst *sdi, size_t ch)
{
	...
	digits -= val_exp;
#endif
	(void)digits; /* PXView's flat analog has no digits field. */
	(void)sig_digits;
	(void)val_exp;
	(void)prec_exp;
	 ...
	 *
	 * PXView: flat analog -- write mq/mqflags/unit/data/unit_bits
	 * directly. The 'digits' value computed above is unused.
	 */
	if (use_double) {
		analog->data = &info->d_value;
		analog->unit_bits = sizeof(info->d_value) * 8;
	} else {
		info->f_value = info->d_value;
		analog->data = &info->f_value;
		analog->unit_bits = sizeof(info->f_value) * 8;
	}
	analog->mq = mq;
	analog->mqflags = mqflag;
	switch (mq) {
	case SR_MQ_VOLTAGE:
		unit = SR_UNIT_VOLT;
		break;
 ...
 * sr_receive_data_callback_t (libsigrok-internal.h:303). The body drops the
	...
	const struct sr_dev_inst *sdi)
{
	...
	for (ch = 0; ch < devc->num_channels; ch++) {
		 ...
		 *
		 * PXView: flat analog -- memset the analog struct to zero
		 * (no sr_analog_init, no encoding/meaning/spec sub-structs).
		 */
		info->packet.type = SR_DF_ANALOG;
		info->packet.status = SR_PKT_OK;
		info->packet.payload = &info->analog[ch];
		memset(&info->analog[ch], 0, sizeof(info->analog[ch]));
		...
		info->analog[ch].probes = g_slist_append(NULL, channel);
		sr_session_send(sdi, &info->packet);
		g_slist_free(info->analog[ch].probes);
		info->analog[ch].probes = NULL;
		sent_sample = TRUE;
	}
	if (sent_sample)
