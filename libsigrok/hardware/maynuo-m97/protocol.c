...
/*
 * Local Modbus RTU support (struct sr_modbus_dev_inst, sr_modbus_* functions)
 * is provided centrally by compat_helpers.c. This file includes compat.h
 * (which pulls in compat_helpers.h) at the top, so all sr_modbus_* symbols
 * are available without a local copy. Previously this file carried its own
 * duplicate copy of the entire Modbus layer, which collided at link time
 * with the copy in rdtech-dps/protocol.c (SR_PRIV is empty on Windows so
 * the symbols were global). The central implementation in compat_helpers.c
 * is the maynuo-m97 version (a superset that also provides
 * sr_modbus_read_coils() and sr_modbus_write_coil()).
 *
 * std_session_send_df_frame_begin/end() are also provided by compat_helpers.c.
 */
