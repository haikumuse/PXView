 ...
 * in a locale-independent way and return SR_OK/SR_ERR. Shared with api.c
 * via the declarations in protocol.h.
 */
...
static void process_line(struct sr_dev_inst *sdi)
{
	...
	case 1: /* Waiting for data reply to request */
		...
		case AQ_I2:
			dbl = 0.0;
			if (lps_sr_atod_ascii(devc->buf, &dbl) != SR_OK) {
		...
		case AQ_STATUS:
			auxint = 0;
			if (lps_sr_atoi(devc->buf, &auxint) != SR_OK) {
	...
	if (revents == G_IO_IN) { /* Serial data arrived. */
		while (LINELEN_MAX - devc->buflen - 2 > 0) {
			...
			if ((devc->buflen > 0) && ((devc->buf[devc->buflen-1] == '\r') || devc->buf[devc->buflen-1] == '\n')) {
				...
				process_line((struct sr_dev_inst *)sdi);
			}
		}
	}
