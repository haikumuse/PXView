...
static int mic_send(struct sr_serial_dev_inst *serial, const char *cmd)
{
	...
	len = strlen(cmd);
	if ((ret = serial_write_blocking(serial, cmd, len,
			MIC_SERIAL_TIMEOUT_MS)) < (int)len) {
...
static int handle_packet(const uint8_t *buf, const struct sr_dev_inst *sdi, int idx)
{
...
static void handle_new_data(const struct sr_dev_inst *sdi, int idx)
{
	struct dev_context *devc;
...
static int receive_data(int fd, int revents, int idx, const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
