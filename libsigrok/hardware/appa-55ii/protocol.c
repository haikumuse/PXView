...
static void appa_55ii_live_data(const struct sr_dev_inst *sdi, const uint8_t *buf)
{
...
static void appa_55ii_log_metadata(const struct sr_dev_inst *sdi, const uint8_t *buf)
{
	struct dev_context *devc;
...
static void appa_55ii_log_data_parse(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
	struct sr_datafeed_packet packet;
	struct sr_datafeed_analog analog;
...
static void appa_55ii_log_data(const struct sr_dev_inst *sdi, const uint8_t *buf)
{
	struct dev_context *devc;
	const uint8_t *ptr;
	unsigned int size;
...
static void appa_55ii_log_end(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
...
static const uint8_t *appa_55ii_parse_data(const struct sr_dev_inst *sdi,
...
SR_PRIV int appa_55ii_receive_data(int fd, int revents,
		const struct sr_dev_inst *sdi)
{
	...
	if (!sdi || !(devc = sdi->priv) || revents != G_IO_IN)
		return TRUE;
	serial = sdi->conn;
