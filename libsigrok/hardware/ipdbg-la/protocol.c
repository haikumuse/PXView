...
SR_PRIV int ipdbg_la_convert_trigger(const struct sr_dev_inst *sdi)
{
	...
	/* PXView compat: sr_session_trigger_get() is a stub that always
	 * returns NULL (no trigger support in PXView). Pass NULL in place
	 * of sdi->session to match the raspberrypi-pico compat pattern. */
...
SR_PRIV int ipdbg_la_receive_data(int fd, int revents, void *cb_data)
{
	...
	if (devc->num_transfers <
		...
		/* Send the trigger. */
		std_session_send_df_trigger(cb_data, NULL);
...
SR_PRIV void ipdbg_la_abort_acquisition(const struct sr_dev_inst *sdi)
{
	...
	/* PXView compat: sr_session_source_remove() takes only the poll object
	 * (no session parameter). */
	sr_session_source_remove(tcp->socket);
