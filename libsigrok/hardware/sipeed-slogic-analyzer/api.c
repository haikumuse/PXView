...
/*
 * Local helper functions for std_str_idx / std_u64_idx / std_i32_idx.
...
static int local_std_i32_idx(GVariant *data, const int32_t vals[], size_t count)
{
	int32_t val;
	size_t i;
   ...
   * or on a USB scan. */
	...
	for (model = support_models_ptr; model->name; model++) {
		...
		for (l = conn_devices; l; l = l->next) {
			...
			sdi = sr_dev_inst_new(SR_INST_USER, SR_ST_INACTIVE,
					       iManufacturer, iProduct, NULL);
...

			{
				...
				devc->limit_samplerate = devc->model->limit_samplerate_table[
					local_std_i32_idx(g_variant_new_int32(devc->limit_samplechannel),
...
static int dev_open(struct sr_dev_inst *sdi)
{
	...
	sr_config_set_compat(sdi, NULL, SR_CONF_VOLTAGE_THRESHOLD,
		      ...
		      const struct sr_channel_group *cg)
{
	...
	case SR_CONF_SAMPLERATE:
		if (g_variant_get_uint64(data) > devc->limit_samplerate ||
		    local_std_u64_idx(data, devc->model->samplerate_table, devc->model->samplerate_table_size) < 0) {
			devc->cur_samplerate = devc->limit_samplerate;
		...
		break;
	case SR_CONF_NUM_LOGIC_CHANNELS:
		if (local_std_i32_idx(data, devc->model->samplechannel_table, devc->model->samplechannel_table_size) < 0) {
			...
			devc->limit_samplerate = devc->model->limit_samplerate_table[
				local_std_i32_idx(g_variant_new_int32(devc->cur_samplechannel),
					devc->model->samplechannel_table, devc->model->samplechannel_table_size)
			];
	...
	case SR_CONF_PATTERN_MODE:
		devc->cur_pattern_mode_idx =
			local_std_str_idx(data, ARRAY_AND_SIZE(patterns));
		       ...
		       const struct sr_channel_group *cg)
{
	...
	case SR_CONF_SAMPLERATE:
		*data = std_gvar_samplerates(
			...
			1 + local_std_u64_idx(g_variant_new_uint64(
	...
	case SR_CONF_TRIGGER_MATCH:
		*data = std_gvar_array_i32(ARRAY_AND_SIZE(trigger_matches));
		break;
	case SR_CONF_VOLTAGE_THRESHOLD:
		*data = std_gvar_min_max_step_thresholds(0, 6, 0.1, NULL, 0);
		break;
	default:
		ret = SR_ERR_NA;
	}
...
static struct sr_dev_driver *sipeed_slogic_drv_ptr;
...
static int sipeed_slogic_compat_init(struct sr_context *sr_ctx)
{
	sipeed_slogic_drv_ptr = &sipeed_slogic_analyzer_driver_info;
	return std_init(sipeed_slogic_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int sipeed_slogic_compat_cleanup(void)
{
	return std_cleanup(sipeed_slogic_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *sipeed_slogic_compat_scan(GSList *options)
{
	return scan(sipeed_slogic_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int sipeed_slogic_compat_config_get(int id, GVariant **data,
...
static int sipeed_slogic_compat_config_set(int id, GVariant *data,
	struct sr_dev_inst *sdi, struct sr_channel *ch,
	struct sr_channel_group *cg)
{
	if (ch != NULL)
		config_channel_set(sdi, ch, SR_CHANNEL_SET_ENABLED);
	return config_set((uint32_t)id, data, sdi, cg);
}

/* Wrapper: PXView config_list(info_id,data,sdi,cg) -> standard(key,data,sdi,cg) */
static int sipeed_slogic_compat_config_list(int info_id, GVariant **data,
...
static int sipeed_slogic_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
	return sipeed_slogic_acquisition_start(sdi);
}

/* Wrapper: PXView acquisition_stop(const sdi, cb_data) -> standard(sdi) */
static int sipeed_slogic_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
	return sipeed_slogic_acquisition_stop((struct sr_dev_inst *)sdi);
}
...
struct sr_dev_driver sipeed_slogic_analyzer_driver_info = {
	...
	.init = sipeed_slogic_compat_init,
	.cleanup = sipeed_slogic_compat_cleanup,
	.scan = sipeed_slogic_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = sipeed_slogic_compat_config_get,
	.config_set = sipeed_slogic_compat_config_set,
	.config_list = sipeed_slogic_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = sipeed_slogic_compat_acquisition_start,
	.dev_acquisition_stop = sipeed_slogic_compat_acquisition_stop,
	.priv = NULL,
};
				   ...
				   const struct sr_dev_inst *sdi)
{
	...
	ds_data_forward(sdi, &(struct sr_datafeed_packet){
				     .type = SR_DF_LOGIC,
