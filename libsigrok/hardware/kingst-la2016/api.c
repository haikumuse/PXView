...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (l = found_devices; l; l = l->next) {
		 ...
		 */
		devc->channel_names_logic = g_malloc0(sizeof(char *) * (ch_max + 1));
		for (ch_idx = 0; ch_idx < ch_max; ch_idx++)
			devc->channel_names_logic[ch_idx] = g_strdup(channel_names_logic[ch_idx]);
...
static struct sr_dev_driver *kingst_la2016_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver kingst_la2016_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int kingst_la2016_compat_init(struct sr_context *sr_ctx)
{
	kingst_la2016_drv_ptr = &kingst_la2016_driver_info;
	return std_init(kingst_la2016_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int kingst_la2016_compat_cleanup(void)
{
	return std_cleanup(kingst_la2016_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *kingst_la2016_compat_scan(GSList *options)
{
	return scan(kingst_la2016_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int kingst_la2016_compat_config_get(int id, GVariant **data,
...
static int kingst_la2016_compat_config_set(int id, GVariant *data,
...
static int kingst_la2016_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int kingst_la2016_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
static int kingst_la2016_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver kingst_la2016_driver_info = {
	.name = "kingst-la2016",
	.longname = "Kingst LA2016",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = kingst_la2016_compat_init,
	.cleanup = kingst_la2016_compat_cleanup,
	.scan = kingst_la2016_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = kingst_la2016_compat_config_get,
	.config_set = kingst_la2016_compat_config_set,
	.config_list = kingst_la2016_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = kingst_la2016_compat_acquisition_start,
	.dev_acquisition_stop = kingst_la2016_compat_acquisition_stop,
	.priv = NULL,
};