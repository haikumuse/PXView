...
/*
 * PXView note: the parameter sdi is const because this is invoked from the
 * receive callback which now receives a const sdi (PXView's
 * sr_receive_data_callback_t signature). ds_data_forward() (which
 * sr_session_send() maps to via compat.h) accepts a const sdi, so this is
 * type-correct without any const-cast.
 *
 * new_analog_struct adaptation (key special-case for this driver):
 * The original standard sigrok source used the new analog API:
 ...
 * mqflags, data, num_samples, unit_bits, ...) -- there is no
 * sr_analog_init() helper and no encoding/meaning/spec sub-structs. Map the
 * meaning.* fields directly onto the flat analog's fields, matching the
 * fluke-dmm / hp-3457a compat pattern. The `digits` argument is informational
 * only (PXView's flat struct has no digits field), so it is marked unused.
 *
 * The original used meaning->channels = sdi->channels (sharing the sdi's
 * channel list directly). PXView's flat struct stores this in `probes`; to
 * avoid the receiving side freeing the device's own channel list, copy the
 * list with g_slist_copy() and free the local copy after sending the packet
 * (matches the fluke-dmm / hp-3457a compat pattern).
 */
    ...
    const struct sr_dev_inst *sdi, double freq_value, int digits)
{
    ...
    (void)digits; /* informational only: PXView flat analog has no digits field */

    memset(&analog, 0, sizeof(analog));
    analog.probes = g_slist_copy((GSList *)sdi->channels);
    analog.data = &freq_value;
    analog.num_samples = 1;
    ...
    /*
     * Original standard sigrok set analog.encoding->unitsize = sizeof(double)
     * (= 8 bytes). PXView's flat sr_datafeed_analog uses unit_bits (in bits),
     * so the equivalent value is 64.
     */
    analog.unit_bits = 64; /* sizeof(double) in bits */
...
static void bkprecision_1856d_parse_message(const struct sr_dev_inst *sdi)
{
    struct dev_context *devc;
    struct sr_serial_dev_inst *serial;
...
/*
 * Rule 14: PXView's sr_receive_data_callback_t passes the sdi directly as the
 * third argument (const struct sr_dev_inst *sdi) instead of the
 * void *cb_data that standard sigrok uses. The body drops the former
 * cb_data dereference and uses sdi directly.
 */
SR_PRIV int bkprecision_1856d_receive_data(int fd, int revents, const struct sr_dev_inst *sdi)
{
    struct dev_context *devc;
    struct sr_serial_dev_inst *serial;
    int len;
