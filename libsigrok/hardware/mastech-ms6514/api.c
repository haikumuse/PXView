...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	if (serial_stream_detect(serial, buf, &len, 2 * MASTECH_MS6514_FRAME_SIZE,
			mastech_ms6514_packet_valid, 500, 9600) != SR_OK)
		goto scan_cleanup;
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	serial_source_add(serial, G_IO_IN, MASTECH_MS6514_BUF_SIZE,
			mastech_ms6514_receive_data, sdi);
...
static struct sr_dev_driver *mastech_ms6514_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver mastech_ms6514_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int mastech_ms6514_compat_init(struct sr_context *sr_ctx)
{
	mastech_ms6514_drv_ptr = &mastech_ms6514_driver_info;
	return std_init(mastech_ms6514_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int mastech_ms6514_compat_cleanup(void)
{
	return std_cleanup(mastech_ms6514_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *mastech_ms6514_compat_scan(GSList *options)
{
	return scan(mastech_ms6514_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int mastech_ms6514_compat_config_get(int id, GVariant **data,
...
static int mastech_ms6514_compat_config_set(int id, GVariant *data,
...
static int mastech_ms6514_compat_config_list(int info_id, GVariant **data,
		const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int mastech_ms6514_compat_acquisition_start(struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
...
static int mastech_ms6514_compat_acquisition_stop(const struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver mastech_ms6514_driver_info = {
	.name = "mastech-ms6514",
	.longname = "MASTECH MS6514",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = mastech_ms6514_compat_init,
	.cleanup = mastech_ms6514_compat_cleanup,
	.scan = mastech_ms6514_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = mastech_ms6514_compat_config_get,
	.config_set = mastech_ms6514_compat_config_set,
	.config_list = mastech_ms6514_compat_config_list,
	.dev_open = std_serial_dev_open,
	.dev_close = std_serial_dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = mastech_ms6514_compat_acquisition_start,
	.dev_acquisition_stop = mastech_ms6514_compat_acquisition_stop,
	.priv = NULL,
};
