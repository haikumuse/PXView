...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (i = 0; devlist[i]; i++) {
		...
		/*
		 * testo_probe_channels() opens the device via the driver's
		 * dev_open callback, so sdi->driver must be set before the
		 * probe call. std_scan_complete_compat() would normally set
		 * this, but it runs after the probe.
		 */
		sdi->driver = di;
 ...
 */
static int handle_events(int fd, int revents, const struct sr_dev_inst *sdi)
{
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	usb_source_add(drvc->sr_ctx, 100, handle_events, sdi);
...
static struct sr_dev_driver *testo_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver testo_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int testo_compat_init(struct sr_context *sr_ctx)
{
	testo_drv_ptr = &testo_driver_info;
	return std_init(testo_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int testo_compat_cleanup(void)
{
	return std_cleanup(testo_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *testo_compat_scan(GSList *options)
{
	return scan(testo_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int testo_compat_config_get(int id, GVariant **data,
...
static int testo_compat_config_set(int id, GVariant *data,
...
static int testo_compat_config_list(int info_id, GVariant **data,
		const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int testo_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
static int testo_compat_acquisition_stop(const struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
 ...
 * The testo devices connect via FTDI-over-USB, so dev_open/dev_close use
 * the custom USB helpers above rather than std_serial_dev_open/close. */
struct sr_dev_driver testo_driver_info = {
	.name = "testo",
	.longname = "Testo 435/635/735",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = testo_compat_init,
	.cleanup = testo_compat_cleanup,
	.scan = testo_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = testo_compat_config_get,
	.config_set = testo_compat_config_set,
	.config_list = testo_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = testo_compat_acquisition_start,
	.dev_acquisition_stop = testo_compat_acquisition_stop,
	.priv = NULL,
};
