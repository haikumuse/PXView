	...
	const struct sr_dev_inst *sdi)
{
	...
	analog1->data = value1;
	analog1->unit_bits = (sizeof(*value1)) * 8;
	analog1->num_samples = 1;
	ch = g_slist_nth_data(sdi->channels, 0);
	analog1->probes = g_slist_append(NULL, ch);
	*ranges1 = NULL;
	mq1 = &analog1->mq;
	mqf1 = &analog1->mqflags;
	unit1 = &analog1->unit;

	/* Prepare general submission on second channel. */
	analog2->data = value2;
	analog2->unit_bits = (sizeof(*value2)) * 8;
	analog2->num_samples = 1;
	ch = g_slist_nth_data(sdi->channels, 1);
	analog2->probes = g_slist_append(NULL, ch);
	*ranges2 = NULL;
	mq2 = &analog2->mq;
	mqf2 = &analog2->mqflags;
	unit2 = &analog2->unit;
 ...
 * @param[in] sdi Device instance (PXView passes sdi directly, not cb_data).
 *
 ...
 *
 * NOTE: PXView's sr_receive_data_callback_t passes the sdi directly as
 * the third argument instead of void *cb_data. The DMM parser state is
 * retrieved from sdi->priv (dev_context) -> dmm -> dmm_state.
 */
static int bm52x_rec_receive_data(int fd, int revents, const struct sr_dev_inst *sdi)
{
	struct brymen_bm52x_state *state;
 ...
 *
 * NOTE: PXView's sr_receive_data_callback_t passes the sdi directly as
 * the third argument instead of void *cb_data. The cb_data parameter
 * was therefore removed from this callback signature. The receive
 * callback retrieves the DMM state from sdi->priv->dmm->dmm_state.
 * The sdi is no longer stored in state (state->sdi was removed) since
 * the callback receives it directly.
 */
