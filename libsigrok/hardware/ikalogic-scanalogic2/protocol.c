...
/*
 * Defined in api.c. The definition is const, so the extern declaration here
 * must match (standard sigrok's original had a non-const extern which was a
 * latent type mismatch).
 */
extern const uint64_t sl2_samplerates[NUM_SAMPLERATES];

static void stop_acquisition(struct sr_dev_inst *sdi)
{
	/*
	 * usb_source_remove() is a compat macro (see protocol.h) that expands to
	 * sr_session_source_remove(-1); its arguments are discarded, so no
	 * drv_context needs to be fetched here.
	 */
	usb_source_remove(sdi->session, NULL);
...
{
	/* See note in stop_acquisition() regarding usb_source_remove(). */
	usb_source_remove(sdi->session, NULL);

	std_session_send_df_end(sdi, LOG_PREFIX);

	/* PXView has no sr_dev_close() helper; dispatch to the driver directly. */
	...
	const struct sr_dev_inst *sdi)
{
	...
	if (ret != 0) {
		...
		abort_acquisition((struct sr_dev_inst *)sdi);
		return TRUE;
	}
