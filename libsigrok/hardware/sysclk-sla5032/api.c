	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_SAMPLERATE:
		...
		idx = -1;
		for (j = 0; j < ARRAY_SIZE(samplerates); j++) {
			if (samplerates[j] == value) {
				idx = (int)j;
				break;
			}
		}
...
static struct sr_dev_driver *sysclk_sla5032_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver sysclk_sla5032_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int sysclk_sla5032_compat_init(struct sr_context *sr_ctx)
{
	sysclk_sla5032_drv_ptr = &sysclk_sla5032_driver_info;
	return std_init(sysclk_sla5032_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int sysclk_sla5032_compat_cleanup(void)
{
	/* Call dev_clear to free per-device resources before cleanup */
	std_dev_clear_compat(sysclk_sla5032_drv_ptr);
	return std_cleanup(sysclk_sla5032_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *sysclk_sla5032_compat_scan(GSList *options)
{
	return scan(sysclk_sla5032_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int sysclk_sla5032_compat_config_get(int id, GVariant **data,
...
static int sysclk_sla5032_compat_config_set(int id, GVariant *data,
...
static int sysclk_sla5032_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int sysclk_sla5032_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
static int sysclk_sla5032_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver sysclk_sla5032_driver_info = {
	.name = "sysclk-sla5032",
	.longname = "Sysclk SLA5032",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = sysclk_sla5032_compat_init,
	.cleanup = sysclk_sla5032_compat_cleanup,
	.scan = sysclk_sla5032_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = sysclk_sla5032_compat_config_get,
	.config_set = sysclk_sla5032_compat_config_set,
	.config_list = sysclk_sla5032_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = sysclk_sla5032_compat_acquisition_start,
	.dev_acquisition_stop = sysclk_sla5032_compat_acquisition_stop,
	.priv = NULL,
};
