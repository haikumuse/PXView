 ...
 *
 * The original sigrok driver used double literals (1e3, 1e6) for the
 * sr_rational.q field, but PXView's local sr_rational uses uint64_t fields.
 * Use integer literals (1000, 1000000) instead to avoid double-to-uint64_t
 * conversion warnings.
 */
static const struct rdtech_um_channel_desc default_channels[] = {
	{ "V",  {   2, BVT_BE_UINT16, }, { 10, 1000, }, 2, SR_MQ_VOLTAGE, SR_UNIT_VOLT },
	{ "I",  {   4, BVT_BE_UINT16, }, {  1, 1000, }, 3, SR_MQ_CURRENT, SR_UNIT_AMPERE },
	{ "D+", {  96, BVT_BE_UINT16, }, { 10, 1000, }, 2, SR_MQ_VOLTAGE, SR_UNIT_VOLT },
	{ "D-", {  98, BVT_BE_UINT16, }, { 10, 1000, }, 2, SR_MQ_VOLTAGE, SR_UNIT_VOLT },
	{ "T",  {  10, BVT_BE_UINT16, }, {  1,    1, }, 0, SR_MQ_TEMPERATURE, SR_UNIT_CELSIUS },
	/* Threshold-based recording (mWh) */
	{ "E",  { 106, BVT_BE_UINT32, }, {  1, 1000, }, 3, SR_MQ_ENERGY, SR_UNIT_WATT_HOUR },
};

static const struct rdtech_um_channel_desc um25c_channels[] = {
	{ "V",  {   2, BVT_BE_UINT16, }, {  1,    1000, }, 3, SR_MQ_VOLTAGE, SR_UNIT_VOLT },
	{ "I",  {   4, BVT_BE_UINT16, }, { 100, 1000000, }, 4, SR_MQ_CURRENT, SR_UNIT_AMPERE },
	{ "D+", {  96, BVT_BE_UINT16, }, { 10,    1000, }, 2, SR_MQ_VOLTAGE, SR_UNIT_VOLT },
	{ "D-", {  98, BVT_BE_UINT16, }, { 10,    1000, }, 2, SR_MQ_VOLTAGE, SR_UNIT_VOLT },
	{ "T",  {  10, BVT_BE_UINT16, }, {  1,       1, }, 0, SR_MQ_TEMPERATURE, SR_UNIT_CELSIUS },
	/* Threshold-based recording (mWh) */
 ...
 * Replaces the original feed_queue_analog_submit_one() call.
 */
static int send_channel_value(const struct sr_dev_inst *sdi,
		const struct rdtech_um_channel_desc *pch, float raw_value,
		size_t ch_idx)
{
		...
		const uint8_t *data, size_t dlen)
{
	...
	for (ch_idx = 0; ch_idx < p->channel_count; ch_idx++) {
		...
		if (ret != SR_OK)
			break;
		ret = send_channel_value(sdi, &p->channels[ch_idx], v, ch_idx);
		if (ret != SR_OK)
			break;
	}
...
SR_PRIV int rdtech_um_receive_data(int fd, int revents, const struct sr_dev_inst *sdi)
{
	...
	serial = sdi->conn;
	if (revents == G_IO_IN) {
		ret = accum_data((struct sr_dev_inst *)sdi, serial);
		if (ret != SR_OK) {
			sr_dev_acquisition_stop(sdi);
			return TRUE;
		}
