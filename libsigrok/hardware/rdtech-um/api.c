...
/*
 * Probe the serial port for an RDTech UM device. On success, returns a
 ...
 * the rdtech_um_profile's channel descriptor table and applied at sample
		...
		const char *conn, const char *serialcomm)
{
	...
	for (ch_idx = 0; ch_idx < p->channel_count; ch_idx++) {
		...
		sr_channel_new(sdi, ch_idx, SR_CHANNEL_ANALOG, TRUE, pch->name);
	}
 ...
 * the DF_END packet. Same pattern as appa-55ii, rdtech-tc, etc.
 */
...
static struct sr_dev_driver *rdtech_um_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver rdtech_um_driver_info;

static int rdtech_um_compat_init(struct sr_context *sr_ctx)
{
	rdtech_um_drv_ptr = &rdtech_um_driver_info;
	return std_init(rdtech_um_drv_ptr, sr_ctx);
}

static int rdtech_um_compat_cleanup(void)
{
	return std_cleanup(rdtech_um_drv_ptr);
}

static GSList *rdtech_um_compat_scan(GSList *options)
{
	return scan(rdtech_um_drv_ptr, options);
}

static int rdtech_um_compat_config_get(int id, GVariant **data,
...
static int rdtech_um_compat_config_set(int id, GVariant *data,
...
static int rdtech_um_compat_config_list(int info_id, GVariant **data,
		const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list((uint32_t)info_id, data, sdi, cg);
}

static int rdtech_um_compat_acquisition_start(struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
...
static int rdtech_um_compat_acquisition_stop(const struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver rdtech_um_driver_info = {
	.name = "rdtech-um",
	.longname = "RDTech UMxx USB power meter",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = rdtech_um_compat_init,
	.cleanup = rdtech_um_compat_cleanup,
	.scan = rdtech_um_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = rdtech_um_compat_config_get,
	.config_set = rdtech_um_compat_config_set,
	.config_list = rdtech_um_compat_config_list,
	.dev_open = std_serial_dev_open,
	.dev_close = std_serial_dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = rdtech_um_compat_acquisition_start,
	.dev_acquisition_stop = rdtech_um_compat_acquisition_stop,
	.priv = NULL,
};
