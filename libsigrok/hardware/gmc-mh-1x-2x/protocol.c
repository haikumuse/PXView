 ...
 * default). Same pattern as colead-slm, atten-pps3xxx, reloadpro.
 */
static int gmc_mh_serial_timeout(struct sr_serial_dev_inst *serial, int bytes)
{
	uint64_t baudrate = 0;
	const char *sc;
...
static int chk_msg14(const struct sr_dev_inst *sdi);
...
static void decode_ctmv_16(uint8_t ctmv, struct dev_context *devc)
{
	...
	case 0x0d: /* 1101 uF (15S/16S only) */
...
static void decode_rs_16(uint8_t rs, struct dev_context *devc)
{
	...
	case 0:
		if (devc->mq == SR_MQ_VOLTAGE) /* V */
			devc->scale += -1;
		else if (devc->mq == SR_MQ_CURRENT) /* 000.0 uA */
			devc->scale += -5;
 ...
 * directly via memset + explicit assignment (matching the hp-3457a /
 * fluke-dmm compat driver pattern). The probes list borrows sdi->channels
 * (not freed here, since it is owned by the device instance).
 */
static void send_value(const struct sr_dev_inst *sdi)
{
...
static void process_msg_dta_6(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
	int cnt;
	uint8_t dgt;
...
static void process_msg_inf_5(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
	enum model model;
...
static void process_msg_inf_10(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
	int cnt;
	uint8_t dgt;
...
static void process_msg_inf_13(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
	enum model model;
	int cnt;
 ...
 * void *cb_data that standard sigrok uses. sr_dev_acquisition_stop() is
 * provided as a static inline by compat.h taking a const sdi.
 */
...
int req_meas14(const struct sr_dev_inst *sdi)
{
	...
	if (serial_write_blocking(serial, msg, sizeof(msg),
			gmc_mh_serial_timeout(serial, sizeof(msg))) < (int)sizeof(msg)) {
		return SR_ERR;
	}
...
int req_stat14(const struct sr_dev_inst *sdi, gboolean power_on)
{
	...
	if (power_on) {
		...
		if (serial_write_blocking(serial, msg, sizeof(msg),
				gmc_mh_serial_timeout(serial, sizeof(msg))) < 0)
			return SR_ERR;
		...
		const struct sr_channel_group *cg)
{
	...
	case SR_CONF_POWER_OFF:
		...
		if (serial_write_blocking(sdi->conn, msg, sizeof(msg),
				gmc_mh_serial_timeout(sdi->conn, sizeof(msg))) < 0)
			return SR_ERR;
		else
 ...
 * port, and send the DF_END packet. Same pattern as colead-slm,
 * gwinstek-gpd. Shared by both gmc_mh_1x_2x_rs232 and gmc_mh_2x_bd232
 * driver_info variants.
 */
