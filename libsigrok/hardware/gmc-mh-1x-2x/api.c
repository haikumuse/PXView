...
/*
 * Forward declarations. Must have external linkage (not static) so that
 * hwdriver.c's drivers_list[] entries `&gmc_mh_1x_2x_rs232_driver_info`
 * and `&gmc_mh_2x_bd232_driver_info` can resolve at link time. The actual
 * definitions are at the end of this file.
 * (Matches the hp-3457a / gwinstek-gpd / colead-slm pattern; the original
...
static GSList *scan_1x_2x_rs232(struct sr_dev_driver *di, GSList *options)
{
	 ...
	 * so manually walk the options list looking for SR_CONF_CONN and
...
static GSList *scan_2x_bd232(struct sr_dev_driver *di, GSList *options)
{
	...
	/*
	 * Manually walk the options list (same pattern as scan_1x_2x_rs232,
	 * colead-slm, gwinstek-gpd).
	 */
	for (l = options; l; l = l->next) {
		src = l->data;
		switch (src->key) {
 ...
 * the 5-arg signature (serial, events, timeout, cb, sdi) — no leading
 * session parameter. std_session_send_df_header() is the 2-arg compat form
 * (sdi, prefix). The callback gmc_mh_1x_2x_receive_data now has PXView's
 * const-sdi signature.
 */
static int dev_acquisition_start_1x_2x_rs232(const struct sr_dev_inst *sdi)
{
	...
	serial = sdi->conn;
	serial_source_add(serial, G_IO_IN, 40,
			gmc_mh_1x_2x_receive_data, sdi);
...
/* ===========================================================================
 * PXView compat wrapper layer — gmc_mh_1x_2x_rs232 (RS-232 variant)
 *
 ...
 * defined above.
 *
 * Note: The original gmc-mh-1x-2x driver does not provide a
...
static struct sr_dev_driver *gmc_mh_1x_2x_rs232_drv_ptr;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int gmc_mh_1x_2x_rs232_compat_init(struct sr_context *sr_ctx)
{
	gmc_mh_1x_2x_rs232_drv_ptr = &gmc_mh_1x_2x_rs232_driver_info;
	return std_init(gmc_mh_1x_2x_rs232_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int gmc_mh_1x_2x_rs232_compat_cleanup(void)
{
	std_dev_clear(gmc_mh_1x_2x_rs232_drv_ptr);
	return std_cleanup(gmc_mh_1x_2x_rs232_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *gmc_mh_1x_2x_rs232_compat_scan(GSList *options)
{
	return scan_1x_2x_rs232(gmc_mh_1x_2x_rs232_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int gmc_mh_1x_2x_rs232_compat_config_get(int id, GVariant **data,
...
static int gmc_mh_1x_2x_rs232_compat_config_set(int id, GVariant *data,
...
static int gmc_mh_1x_2x_rs232_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list_sm((uint32_t)info_id, data, sdi, cg);
}

/* Wrapper: PXView acquisition_start(sdi, cb_data) -> standard(sdi) */
static int gmc_mh_1x_2x_rs232_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
	return dev_acquisition_start_1x_2x_rs232(sdi);
}

/* Wrapper: PXView acquisition_stop(const sdi, cb_data) -> standard(sdi) */
static int gmc_mh_1x_2x_rs232_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
	return gmc_mh_1x_2x_dev_acquisition_stop((struct sr_dev_inst *)sdi);
}

/* PXView-compatible driver info struct — RS-232 variant */
struct sr_dev_driver gmc_mh_1x_2x_rs232_driver_info = {
	.name = "gmc-mh-1x-2x-rs232",
	.longname = "Gossen Metrawatt Metrahit 1x/2x, RS232 interface",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = gmc_mh_1x_2x_rs232_compat_init,
	.cleanup = gmc_mh_1x_2x_rs232_compat_cleanup,
	.scan = gmc_mh_1x_2x_rs232_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = gmc_mh_1x_2x_rs232_compat_config_get,
	.config_set = gmc_mh_1x_2x_rs232_compat_config_set,
	.config_list = gmc_mh_1x_2x_rs232_compat_config_list,
	.dev_open = std_serial_dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = gmc_mh_1x_2x_rs232_compat_acquisition_start,
	.dev_acquisition_stop = gmc_mh_1x_2x_rs232_compat_acquisition_stop,
	.priv = NULL,
};

/* ===========================================================================
 * PXView compat wrapper layer — gmc_mh_2x_bd232 (BD232 variant)
 *
 * Shares dev_open/dev_close/config_get/config_set/dev_acquisition_stop with
 * the RS-232 variant, but has its own scan, config_list, and
 * dev_acquisition_start. Each driver_info still carries its own complete set
 * of 8 compat wrappers per the migration spec.
 * ========================================================================== */

/* Static driver pointer for wrapper functions */
static struct sr_dev_driver *gmc_mh_2x_bd232_drv_ptr;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int gmc_mh_2x_bd232_compat_init(struct sr_context *sr_ctx)
{
	gmc_mh_2x_bd232_drv_ptr = &gmc_mh_2x_bd232_driver_info;
	return std_init(gmc_mh_2x_bd232_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int gmc_mh_2x_bd232_compat_cleanup(void)
{
	std_dev_clear(gmc_mh_2x_bd232_drv_ptr);
	return std_cleanup(gmc_mh_2x_bd232_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *gmc_mh_2x_bd232_compat_scan(GSList *options)
{
	return scan_2x_bd232(gmc_mh_2x_bd232_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int gmc_mh_2x_bd232_compat_config_get(int id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel *ch,
	const struct sr_channel_group *cg)
{
	(void)ch;
...
/* Wrapper: PXView config_set(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int gmc_mh_2x_bd232_compat_config_set(int id, GVariant *data,
	struct sr_dev_inst *sdi, struct sr_channel *ch,
	struct sr_channel_group *cg)
{
	(void)ch;
...
/* Wrapper: PXView config_list(info_id,data,sdi,cg) -> standard(key,data,sdi,cg) */
static int gmc_mh_2x_bd232_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list_bd((uint32_t)info_id, data, sdi, cg);
}

/* Wrapper: PXView acquisition_start(sdi, cb_data) -> standard(sdi) */
static int gmc_mh_2x_bd232_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
	return dev_acquisition_start_2x_bd232(sdi);
}

/* Wrapper: PXView acquisition_stop(const sdi, cb_data) -> standard(sdi) */
static int gmc_mh_2x_bd232_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
	return gmc_mh_1x_2x_dev_acquisition_stop((struct sr_dev_inst *)sdi);
...
/* PXView-compatible driver info struct — BD232 variant */
struct sr_dev_driver gmc_mh_2x_bd232_driver_info = {
	.name = "gmc-mh-2x-bd232",
	.longname = "Gossen Metrawatt Metrahit 2x, BD232/SI232-II interface",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = gmc_mh_2x_bd232_compat_init,
	.cleanup = gmc_mh_2x_bd232_compat_cleanup,
	.scan = gmc_mh_2x_bd232_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = gmc_mh_2x_bd232_compat_config_get,
	.config_set = gmc_mh_2x_bd232_compat_config_set,
	.config_list = gmc_mh_2x_bd232_compat_config_list,
	.dev_open = std_serial_dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = gmc_mh_2x_bd232_compat_acquisition_start,
	.dev_acquisition_stop = gmc_mh_2x_bd232_compat_acquisition_stop,
	.priv = NULL,
};
