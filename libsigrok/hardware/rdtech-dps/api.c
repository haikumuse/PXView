 ...
 * at the bottom of this file. Referenced by probe_device() to pick the
 * correct driver variant for the probed device. */
	...
	enum rdtech_dps_model_type model_type)
{
	...
	/*
	 * Standard sigrok used di->context (which pointed to the driver_info
	 * struct) to distinguish DPS vs RD variants. PXView uses di->priv for
	 * the drv_context instead, so we rely solely on the model_type
	 * parameter that scan_dps/scan_rd pass in. This is safe because the
	 * only callers are scan_dps (MODEL_DPS) and scan_rd (MODEL_RD).
	 */
	serialcomm = NULL;
	probe_func = NULL;
	if (model_type == MODEL_DPS) {
		serialcomm = default_serialcomm_dps;
		probe_func = probe_device_dps;
	}
	if (model_type == MODEL_RD) {
	...
	devices = sr_modbus_scan(di, opts, probe_func);
...
/*
 * dev_open / dev_close: the rdtech-dps driver talks over Modbus RTU
 * (sdi->conn is a struct sr_modbus_dev_inst), so the standard
 * std_serial_dev_open/close helpers cannot be used here. We keep the
 * original open/close logic that calls sr_modbus_open/close and manages
	...
	case SR_CONF_DEVICE_OPTIONS:
		if (devc && devc->model->n_ranges > 1) {
			return std_config_list(key, data, sdi, cg,
				...
				devopts_w_range, ARRAY_SIZE(devopts_w_range));
		} else {
			return std_config_list(key, data, sdi, cg,
				scanopts, ARRAY_SIZE(scanopts),
				drvopts, ARRAY_SIZE(drvopts),
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	ret = sr_modbus_source_add(modbus, G_IO_IN, 10,
			rdtech_dps_receive_data, sdi);
	if (ret != SR_OK) {
		devc->acquisition_started = FALSE;
		return ret;
	}
 ...
 *
 * The rdtech-dps driver registers two driver variants: "rdtech-dps" (DPS/DPH
 * family) and "rdtech-rd" (RD family). Each gets its own set of wrapper
 * functions and its own driver_info struct.
 * =========================================================================== */
...
static struct sr_dev_driver *rdtech_dps_drv_ptr;

static int rdtech_dps_compat_init(struct sr_context *sr_ctx)
{
	rdtech_dps_drv_ptr = &rdtech_dps_driver_info;
	return std_init(rdtech_dps_drv_ptr, sr_ctx);
}

static int rdtech_dps_compat_cleanup(void)
{
	return std_cleanup(rdtech_dps_drv_ptr);
}

static GSList *rdtech_dps_compat_scan(GSList *options)
{
	return scan_dps(rdtech_dps_drv_ptr, options);
}

static int rdtech_dps_compat_config_get(int id, GVariant **data,
...
static int rdtech_dps_compat_config_set(int id, GVariant *data,
...
static int rdtech_dps_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list((uint32_t)info_id, data, sdi, cg);
}

static int rdtech_dps_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
static int rdtech_dps_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver rdtech_dps_driver_info = {
	...
	.init = rdtech_dps_compat_init,
	.cleanup = rdtech_dps_compat_cleanup,
	.scan = rdtech_dps_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = rdtech_dps_compat_config_get,
	.config_set = rdtech_dps_compat_config_set,
	.config_list = rdtech_dps_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = rdtech_dps_compat_acquisition_start,
	.dev_acquisition_stop = rdtech_dps_compat_acquisition_stop,
	.priv = NULL,
};
...
static struct sr_dev_driver *rdtech_rd_drv_ptr;

static int rdtech_rd_compat_init(struct sr_context *sr_ctx)
{
	rdtech_rd_drv_ptr = &rdtech_rd_driver_info;
	return std_init(rdtech_rd_drv_ptr, sr_ctx);
}

static int rdtech_rd_compat_cleanup(void)
{
	return std_cleanup(rdtech_rd_drv_ptr);
}

static GSList *rdtech_rd_compat_scan(GSList *options)
{
	return scan_rd(rdtech_rd_drv_ptr, options);
}

static int rdtech_rd_compat_config_get(int id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel *ch,
	const struct sr_channel_group *cg)
{
	(void)ch;
...
static int rdtech_rd_compat_config_set(int id, GVariant *data,
	struct sr_dev_inst *sdi, struct sr_channel *ch,
	struct sr_channel_group *cg)
{
	(void)ch;
...
static int rdtech_rd_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list((uint32_t)info_id, data, sdi, cg);
}

static int rdtech_rd_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
	return dev_acquisition_start(sdi);
...
static int rdtech_rd_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
	return dev_acquisition_stop((struct sr_dev_inst *)sdi);
...
struct sr_dev_driver rdtech_rd_driver_info = {
	...
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = rdtech_rd_compat_init,
	.cleanup = rdtech_rd_compat_cleanup,
	.scan = rdtech_rd_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = rdtech_rd_compat_config_get,
	.config_set = rdtech_rd_compat_config_set,
	.config_list = rdtech_rd_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = rdtech_rd_compat_acquisition_start,
	.dev_acquisition_stop = rdtech_rd_compat_acquisition_stop,
	.priv = NULL,
};
