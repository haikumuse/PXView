 ...
 * libsigrok does not provide. The HP/Keysight status register parsers
 * in api.c call scpi_pps_sr_atoi() to decode numeric register responses.
 * Implemented here (with SR_PRIV linkage) so api.c can share it.
 */
 ...
 * libsigrok does not provide. Declared static (file-local) so it cannot
 * collide with copies in other compat drivers (rigol-ds, siglent-sds) at
 * link time. Used by ea_ps_20xx_probe_channels() below.
 */
static int scpi_pps_sr_atof_ascii(const char *str, float *ret)
{
	char *endptr;
	double val;
	...
	if (errno != 0 || endptr == str)
		return SR_ERR;
	/* Tolerate trailing whitespace (standard sigrok behaviour). */
	while (*endptr != '\0') {
		if (!g_ascii_isspace(*endptr))
			return SR_ERR;
		endptr++;
		...
		unsigned int *num_channel_groups)
{
	...
	/* Remove unit */
	response[ret] = 0;
	if (scpi_pps_sr_atof_ascii(response, &nom_volt) != SR_OK) {
	...
	/* Remove unit */
	response[ret] = 0;
	if (scpi_pps_sr_atof_ascii(response, &nom_curr) != SR_OK) {
	...
	/* Remove unit */
	response[ret] = 0;
	if (scpi_pps_sr_atof_ascii(response, &nom_pow) != SR_OK) {
