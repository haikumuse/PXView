...
/*
 * Local helper that converts a 5-element channel_spec tuple
 * {min, max, step, spec_digits, encoding_digits} into a GVariant
 * (min, max, step) tuple. Standard sigrok provides std_gvar_min_max_step_array()
 * for this; PXView's compat layer only has std_gvar_min_max_step(min, max, step),
 * so wrap it.
 */
static GVariant *local_std_gvar_min_max_step_array(const double *arr)
{
	return std_gvar_min_max_step(arr[0], arr[1], arr[2]);
}
...
static struct sr_dev_inst *probe_device(struct sr_scpi_dev_inst *scpi,
		...
		struct sr_dev_driver *driver_info)
{
...
{
	return probe_device(scpi, sr_scpi_get_hw_id, &scpi_pps_driver_info);
}

static struct sr_dev_inst *probe_hpib_pps_device(struct sr_scpi_dev_inst *scpi)
{
	return probe_device(scpi, hpib_get_hw_id, &hp_ib_pps_driver_info);
}

static GSList *scan_scpi_pps(struct sr_dev_driver *di, GSList *options)
{
	return sr_scpi_scan((struct drv_context *)di->priv, options,
			probe_scpi_pps_device);
}
...
static GSList *scan_hpib_pps(struct sr_dev_driver *di, GSList *options)
{
	 ...
	 * manually walk the options list looking for SR_CONF_CONN (same pattern
	 * as motech-lps-30x/api.c).
	 */
	conn = NULL;
...
/*
 * Standard sigrok's std_dev_clear_with_callback() is not available in PXView's
 * compat layer. Inline the clear_helper logic: iterate instances, call
 * clear_helper on each devc, then use std_dev_clear_compat to free everything
 * (same pattern as lecroy-xstream/hameg-hmo/siglent-sds compat drivers).
 */
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	if (!cg) {
		...
		case SR_CONF_DEVICE_OPTIONS:
			/*
			 * Standard sigrok used std_opts_config_list() here. PXView's
			 * compat layer provides std_config_list() with the same
			 * semantics (just a different name), so use it directly.
			 * Pass the per-device devopts array from the matched profile
			 * (may be NULL during early scan).
			 */
		...
		case SR_CONF_DEVICE_OPTIONS:
			*data = std_gvar_array_u32(devc->device->devopts_cg,
			...
			break;
		case SR_CONF_VOLTAGE_TARGET:
			*data = local_std_gvar_min_max_step_array(ch_spec->voltage);
			break;
		case SR_CONF_OUTPUT_FREQUENCY_TARGET:
			*data = local_std_gvar_min_max_step_array(ch_spec->frequency);
			break;
		case SR_CONF_CURRENT_LIMIT:
			*data = local_std_gvar_min_max_step_array(ch_spec->current);
			break;
		case SR_CONF_OVER_VOLTAGE_PROTECTION_THRESHOLD:
			*data = local_std_gvar_min_max_step_array(ch_spec->ovp);
			break;
		case SR_CONF_OVER_CURRENT_PROTECTION_THRESHOLD:
			*data = local_std_gvar_min_max_step_array(ch_spec->ocp);
			break;
		case SR_CONF_OVER_CURRENT_PROTECTION_DELAY:
			*data = local_std_gvar_min_max_step_array(ch_spec->ocp_delay);
			break;
		default:
			return SR_ERR_NA;
		}
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	devc->cur_acquisition_channel = scpi_pps_next_enabled_channel(sdi, NULL);
 ...
 *
 * The scpi-pps driver registers two driver variants (scpi-pps for SCPI
 * devices, hpib-pps for legacy HP-IB devices). Each variant needs its own
 * set of wrapper functions and its own driver_info struct, following the
 * same pattern as rdtech-dps (rdtech_dps_driver_info + rdtech_rd_driver_info).
 * ===========================================================================
 */
...
static struct sr_dev_driver *scpi_pps_drv_ptr;

static int scpi_pps_compat_init(struct sr_context *sr_ctx)
{
	scpi_pps_drv_ptr = &scpi_pps_driver_info;
	return std_init(scpi_pps_drv_ptr, sr_ctx);
}

static int scpi_pps_compat_cleanup(void)
{
	return std_cleanup(scpi_pps_drv_ptr);
}

static GSList *scpi_pps_compat_scan(GSList *options)
{
	return scan_scpi_pps(scpi_pps_drv_ptr, options);
}

static int scpi_pps_compat_config_get(int id, GVariant **data,
...
static int scpi_pps_compat_config_set(int id, GVariant *data,
...
static int scpi_pps_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list((uint32_t)info_id, data, sdi, cg);
}

static int scpi_pps_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
static int scpi_pps_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver scpi_pps_driver_info = {
	...
	.init = scpi_pps_compat_init,
	.cleanup = scpi_pps_compat_cleanup,
	.scan = scpi_pps_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = scpi_pps_compat_config_get,
	.config_set = scpi_pps_compat_config_set,
	.config_list = scpi_pps_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = scpi_pps_compat_acquisition_start,
	.dev_acquisition_stop = scpi_pps_compat_acquisition_stop,
	.priv = NULL,
};
...
static struct sr_dev_driver *hp_ib_pps_drv_ptr;

static int hp_ib_pps_compat_init(struct sr_context *sr_ctx)
{
	hp_ib_pps_drv_ptr = &hp_ib_pps_driver_info;
	return std_init(hp_ib_pps_drv_ptr, sr_ctx);
}

static int hp_ib_pps_compat_cleanup(void)
{
	return std_cleanup(hp_ib_pps_drv_ptr);
}

static GSList *hp_ib_pps_compat_scan(GSList *options)
{
	return scan_hpib_pps(hp_ib_pps_drv_ptr, options);
}

static int hp_ib_pps_compat_config_get(int id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel *ch,
	const struct sr_channel_group *cg)
{
	(void)ch;
...
static int hp_ib_pps_compat_config_set(int id, GVariant *data,
	struct sr_dev_inst *sdi, struct sr_channel *ch,
	struct sr_channel_group *cg)
{
	(void)ch;
...
static int hp_ib_pps_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list((uint32_t)info_id, data, sdi, cg);
}

static int hp_ib_pps_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
	return dev_acquisition_start(sdi);
...
static int hp_ib_pps_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
	return dev_acquisition_stop((struct sr_dev_inst *)sdi);
...
struct sr_dev_driver hp_ib_pps_driver_info = {
	...
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = hp_ib_pps_compat_init,
	.cleanup = hp_ib_pps_compat_cleanup,
	.scan = hp_ib_pps_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = hp_ib_pps_compat_config_get,
	.config_set = hp_ib_pps_compat_config_set,
	.config_list = hp_ib_pps_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = hp_ib_pps_compat_acquisition_start,
	.dev_acquisition_stop = hp_ib_pps_compat_acquisition_stop,
	.priv = NULL,
};
