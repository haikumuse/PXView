			      ...
			      uint8_t command)
{
	...
	buf[0] = command;
	if (serial_write_blocking(serial, buf, 1, ols_serial_timeout(serial, 1)) != 1)
		return SR_ERR;
			     ...
			     uint8_t *data)
{
	...
	buf[3] = data[2];
	buf[4] = data[3];
	if (serial_write_blocking(serial, buf, 5, ols_serial_timeout(serial, 1)) != 5)
		return SR_ERR;
...
SR_PRIV int ols_get_metadata(struct sr_dev_inst *sdi)
{
	...
	key = 0xff;
	while (key) {
		delay_ms = ols_serial_timeout(serial, 1);
		...
		case 1:
			...
			delay_ms = ols_serial_timeout(serial, 4);
