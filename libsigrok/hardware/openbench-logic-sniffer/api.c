...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (i = 0; i < ch_max; i++)
		devc->channel_names[i] = g_strdup(ols_channel_names[i]);
	...
	if (ols_set_samplerate(sdi, DEFAULT_SAMPLERATE) != SR_OK)
		sr_dbg("Failed to set default samplerate.");
		       ...
		       const struct sr_channel_group *cg)
{
	...
	case SR_CONF_TRIGGER_MATCH:
		*data = std_gvar_array_i32(ARRAY_AND_SIZE(trigger_matches));
		break;
	case SR_CONF_CLOCK_EDGE:
		*data = g_variant_new_strv(ARRAY_AND_SIZE(external_clock_edges));
		break;
	case SR_CONF_PATTERN_MODE:
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	return serial_source_add(serial, G_IO_IN, 100,
				 ols_receive_data, (const struct sr_dev_inst *)sdi);
}
...
static struct sr_dev_driver *ols_drv_ptr;

extern struct sr_dev_driver ols_driver_info;

static int ols_compat_init(struct sr_context *sr_ctx)
{
	ols_drv_ptr = &ols_driver_info;
	return std_init(ols_drv_ptr, sr_ctx);
}

static int ols_compat_cleanup(void)
{
	std_dev_clear_compat(ols_drv_ptr);
	return std_cleanup(ols_drv_ptr);
}

static GSList *ols_compat_scan(GSList *options)
{
	return scan(ols_drv_ptr, options);
}

static int ols_compat_config_get(int id, GVariant **data,
...
static int ols_compat_config_set(int id, GVariant *data,
...
static int ols_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return config_list((uint32_t)info_id, data, sdi, cg);
}

static int ols_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
	return dev_acquisition_start(sdi);
}

static int ols_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver ols_driver_info = {
	...
	.init = ols_compat_init,
	.cleanup = ols_compat_cleanup,
	.scan = ols_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = ols_compat_config_get,
	.config_set = ols_compat_config_set,
	.config_list = ols_compat_config_list,
	.dev_open = std_serial_dev_open,
	.dev_close = std_serial_dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = ols_compat_acquisition_start,
	.dev_acquisition_stop = ols_compat_acquisition_stop,
	.priv = NULL,
};