...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (i = 0; devlist[i]; i++) {
		...
		case LOGICSTUDIO16_PID_LACK_FIRMWARE:
			r = ezusb_upload_firmware(devlist[i],
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_DEVICE_OPTIONS:
		/*
		 * lecroy-logicstudio exposes no scan options, so pass a NULL
		 * array with a zero count to std_config_list directly (the
		 * STD_CONFIG_LIST macro is replaced by this direct call per
		 * the PXView compat layer conventions).
		 */
...
static int receive_usb_data(int fd, int revents, const struct sr_dev_inst *sdi)
{
	struct drv_context *drvc;
	struct timeval tv;
	...
	/*
	 * PXView's sr_receive_data_callback_t passes the sdi as the callback
	 * data; retrieve the drv_context from the driver's priv pointer.
	 */
	drvc = sdi->driver->priv;
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	 ...
	 */
	return usb_source_add(drvc->sr_ctx, 100, receive_usb_data, sdi);
}
...
static struct sr_dev_driver *lecroy_logicstudio_drv_ptr;
 ...
 * Sets the static driver pointer first, then delegates to std_init().
 */
static int lecroy_logicstudio_compat_init(struct sr_context *sr_ctx)
{
	lecroy_logicstudio_drv_ptr = &lecroy_logicstudio_driver_info;
	return std_init(lecroy_logicstudio_drv_ptr, sr_ctx);
}

/*
 * Wrapper: PXView cleanup(void) -> standard cleanup(driver).
 * Clears all device instances first via std_dev_clear_compat, then
 * frees the driver private context via std_cleanup().
 */
static int lecroy_logicstudio_compat_cleanup(void)
{
	std_dev_clear_compat(lecroy_logicstudio_drv_ptr);
	return std_cleanup(lecroy_logicstudio_drv_ptr);
}

/*
 * Wrapper: PXView scan(options) -> standard scan(driver, options).
 * Delegates to the internal scan() and finalises via
 * std_scan_complete_compat() so discovered devices get registered in the
 * driver's instance list.
 */
static GSList *lecroy_logicstudio_compat_scan(GSList *options)
{
	GSList *devices;

	devices = scan(lecroy_logicstudio_drv_ptr, options);
	return std_scan_complete_compat(lecroy_logicstudio_drv_ptr, devices);
}

/*
 * Wrapper: PXView config_get(id, data, sdi, ch, cg) ->
 * standard config_get(key, data, sdi, cg).
 * Drops the channel parameter (ch) and casts the int id to uint32_t.
 */
static int lecroy_logicstudio_compat_config_get(int id, GVariant **data,
...
/*
 * Wrapper: PXView config_set(id, data, sdi, ch, cg) ->
 * standard config_set(key, data, sdi, cg).
 * Drops the channel parameter (ch) and casts the int id to uint32_t.
 */
static int lecroy_logicstudio_compat_config_set(int id, GVariant *data,
...
/*
 * Wrapper: PXView config_list(info_id, data, sdi, cg) ->
 * standard config_list(key, data, sdi, cg).
 * Casts the int info_id to uint32_t.
 */
static int lecroy_logicstudio_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
 ...
 * standard dev_acquisition_start(sdi).
 * Drops the cb_data parameter.
 */
static int lecroy_logicstudio_compat_acquisition_start(
 ...
 * standard dev_acquisition_stop(sdi).
 * Drops cb_data and casts away const.
 */
static int lecroy_logicstudio_compat_acquisition_stop(
 ...
 *
 * driver_type is set to DRIVER_TYPE_HARDWARE so libsigrok routes scan/open
 * calls to this driver. dev_mode_list/dev_destroy/dev_status_get use the
 * compat-layer defaults since lecroy-logicstudio has no special needs.
 * priv (not context) holds the struct drv_context after init().
 */
struct sr_dev_driver lecroy_logicstudio_driver_info = {
	.name = "lecroy-logicstudio",
	.longname = "LeCroy LogicStudio",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = lecroy_logicstudio_compat_init,
	.cleanup = lecroy_logicstudio_compat_cleanup,
	.scan = lecroy_logicstudio_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = lecroy_logicstudio_compat_config_get,
	.config_set = lecroy_logicstudio_compat_config_set,
	.config_list = lecroy_logicstudio_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = lecroy_logicstudio_compat_acquisition_start,
	.dev_acquisition_stop = lecroy_logicstudio_compat_acquisition_stop,
	.priv = NULL,
};
