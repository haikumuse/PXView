...
/*
 * All Center 3xx devices use a fixed 9600 baud serial link. PXView's
 * compat serial_timeout() helper takes an explicit baudrate argument
 * (3-arg signature) whereas standard sigrok's version derives it from
 * the serial handle (2-arg). Use the known baudrate here.
 */
#define CENTER_3XX_BAUDRATE 9600
...
static int center_send(struct sr_serial_dev_inst *serial, const char *cmd)
{
	...
	if ((ret = serial_write_blocking(serial, cmd, strlen(cmd),
			serial_timeout(serial, CENTER_3XX_BAUDRATE,
					(int)strlen(cmd)))) < 0) {
...
static int receive_data(int fd, int revents, int idx,
...
SR_PRIV int receive_data_##ID_UPPER(int fd, int revents, \
		const struct sr_dev_inst *sdi) { \
	return receive_data(fd, revents, ID_UPPER, sdi); }
