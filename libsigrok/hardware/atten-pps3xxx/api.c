 ...
 */
static int atten_str_idx(GVariant *data, const char *const strs[], size_t count)
{
	const char *s;
	size_t i;
...
static GSList *scan(struct sr_dev_driver *di, GSList *options, int modelid)
{
	...
	packet[0] = 0xaa;
	packet[1] = 0xaa;
	delay_ms = atten_serial_timeout(serial, PACKET_SIZE);
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	if (!cg) {
		switch (key) {
		case SR_CONF_CHANNEL_CONFIG:
			if ((ival = atten_str_idx(data, ARRAY_AND_SIZE(channel_modes))) < 0)
				return SR_ERR_ARG;
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	serial_source_add(serial, G_IO_IN, 50,
			atten_pps3xxx_receive_data, sdi);
...
static struct sr_dev_driver *atten_pps3xxx_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver atten_pps3xxx_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int atten_pps3xxx_compat_init(struct sr_context *sr_ctx)
{
	atten_pps3xxx_drv_ptr = &atten_pps3xxx_driver_info;
	return std_init(atten_pps3xxx_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int atten_pps3xxx_compat_cleanup(void)
{
	return std_cleanup(atten_pps3xxx_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *atten_pps3xxx_compat_scan(GSList *options)
{
	return scan_3203(atten_pps3xxx_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int atten_pps3xxx_compat_config_get(int id, GVariant **data,
...
static int atten_pps3xxx_compat_config_set(int id, GVariant *data,
...
static int atten_pps3xxx_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int atten_pps3xxx_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
static int atten_pps3xxx_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver atten_pps3xxx_driver_info = {
	.name = "atten-pps3xxx",
	.longname = "Atten PPS3xxx",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = atten_pps3xxx_compat_init,
	.cleanup = atten_pps3xxx_compat_cleanup,
	.scan = atten_pps3xxx_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = atten_pps3xxx_compat_config_get,
	.config_set = atten_pps3xxx_compat_config_set,
	.config_list = atten_pps3xxx_compat_config_list,
	.dev_open = std_serial_dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = atten_pps3xxx_compat_acquisition_start,
	.dev_acquisition_stop = atten_pps3xxx_compat_acquisition_stop,
	.priv = NULL,
};
