...
static struct sr_dev_driver *sysclk_lwla_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver sysclk_lwla_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int sysclk_lwla_compat_init(struct sr_context *sr_ctx)
{
	sysclk_lwla_drv_ptr = &sysclk_lwla_driver_info;
	return std_init(sysclk_lwla_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int sysclk_lwla_compat_cleanup(void)
{
	return std_cleanup(sysclk_lwla_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *sysclk_lwla_compat_scan(GSList *options)
{
	return scan(sysclk_lwla_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int sysclk_lwla_compat_config_get(int id, GVariant **data,
...
static int sysclk_lwla_compat_config_set(int id, GVariant *data,
...
static int sysclk_lwla_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int sysclk_lwla_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
static int sysclk_lwla_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver sysclk_lwla_driver_info = {
	.name = "sysclk-lwla",
	.longname = "Sysclk LWLA series",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = sysclk_lwla_compat_init,
	.cleanup = sysclk_lwla_compat_cleanup,
	.scan = sysclk_lwla_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = sysclk_lwla_compat_config_get,
	.config_set = sysclk_lwla_compat_config_set,
	.config_list = sysclk_lwla_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = sysclk_lwla_compat_acquisition_start,
	.dev_acquisition_stop = sysclk_lwla_compat_acquisition_stop,
	.priv = NULL,
};