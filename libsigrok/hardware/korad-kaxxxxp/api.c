...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	serial_source_add(serial, G_IO_IN,
		...
		korad_kaxxxxp_receive_data, sdi);
...
static struct sr_dev_driver *korad_kaxxxxp_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver korad_kaxxxxp_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int korad_kaxxxxp_compat_init(struct sr_context *sr_ctx)
{
	korad_kaxxxxp_drv_ptr = &korad_kaxxxxp_driver_info;
	return std_init(korad_kaxxxxp_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int korad_kaxxxxp_compat_cleanup(void)
{
	return std_cleanup(korad_kaxxxxp_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *korad_kaxxxxp_compat_scan(GSList *options)
{
	return scan(korad_kaxxxxp_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int korad_kaxxxxp_compat_config_get(int id, GVariant **data,
...
static int korad_kaxxxxp_compat_config_set(int id, GVariant *data,
...
static int korad_kaxxxxp_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int korad_kaxxxxp_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
static int korad_kaxxxxp_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver korad_kaxxxxp_driver_info = {
	.name = "korad-kaxxxxp",
	.longname = "Korad KAxxxxP",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = korad_kaxxxxp_compat_init,
	.cleanup = korad_kaxxxxp_compat_cleanup,
	.scan = korad_kaxxxxp_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = korad_kaxxxxp_compat_config_get,
	.config_set = korad_kaxxxxp_compat_config_set,
	.config_list = korad_kaxxxxp_compat_config_list,
	.dev_open = std_serial_dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = korad_kaxxxxp_compat_acquisition_start,
	.dev_acquisition_stop = korad_kaxxxxp_compat_acquisition_stop,
	.priv = NULL,
};
