 ...
 * directly (matching the fluke-dmm compat driver pattern). The num_digits
 * value computed below is informational only (PXView's analog struct has no
 * digits field); the calculation is kept to preserve the original driver's
 * precision logic, but the result is marked unused.
 */
static void acq_send_measurement(const struct sr_dev_inst *sdi)
{
	...
	/*
	 * Original used meaning.channels = g_slist_append(NULL, current_channel)
	 * via the new sigrok analog API. Map to the old flat struct's probes
	 * field and free it after sending the packet.
	 */
	analog.probes = g_slist_append(NULL, devc->current_channel);
	...
	analog.mqflags = devc->measurement_mq_flags;
