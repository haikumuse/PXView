...
static int config_list(uint32_t key, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	return std_config_list(key, data, sdi, cg, scanopts, ARRAY_SIZE(scanopts),
		drvopts, ARRAY_SIZE(drvopts), devopts, ARRAY_SIZE(devopts));
}
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	usb_source_add(drvc->sr_ctx, 100,
			lascar_el_usb_handle_events, sdi);
...
static struct sr_dev_driver *lascar_el_usb_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver lascar_el_usb_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int lascar_el_usb_compat_init(struct sr_context *sr_ctx)
{
	lascar_el_usb_drv_ptr = &lascar_el_usb_driver_info;
	return std_init(lascar_el_usb_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int lascar_el_usb_compat_cleanup(void)
{
	return std_cleanup(lascar_el_usb_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *lascar_el_usb_compat_scan(GSList *options)
{
	return scan(lascar_el_usb_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int lascar_el_usb_compat_config_get(int id, GVariant **data,
...
static int lascar_el_usb_compat_config_set(int id, GVariant *data,
...
static int lascar_el_usb_compat_config_list(int info_id, GVariant **data,
		const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int lascar_el_usb_compat_acquisition_start(struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
...
static int lascar_el_usb_compat_acquisition_stop(const struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
 ...
 * The Lascar EL-USB devices connect via raw USB (bulk + control transfers,
 * no FTDI chip), so dev_open/dev_close use the custom USB helpers above
 * rather than std_serial_dev_open/close. */
struct sr_dev_driver lascar_el_usb_driver_info = {
	.name = "lascar-el-usb",
	.longname = "Lascar EL-USB",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = lascar_el_usb_compat_init,
	.cleanup = lascar_el_usb_compat_cleanup,
	.scan = lascar_el_usb_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = lascar_el_usb_compat_config_get,
	.config_set = lascar_el_usb_compat_config_set,
	.config_list = lascar_el_usb_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = lascar_el_usb_compat_acquisition_start,
	.dev_acquisition_stop = lascar_el_usb_compat_acquisition_stop,
	.priv = NULL,
};
