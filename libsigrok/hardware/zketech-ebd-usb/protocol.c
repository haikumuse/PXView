 ...
 * parse it from the serialcomm string (format like "9600/8e1"). Falls back
 * to 0 when parsing fails (the compat layer will pick a conservative
 * default).
 */
static int ebd_serial_timeout(struct sr_serial_dev_inst *serial, int bytes)
{
	uint64_t baudrate = 0;
	const char *sc;
	...
	uint8_t *buf)
{
	...
	while (!message_complete && turn < max_turns) {
		...
		while ((buf[0] != MSG_FRAME_BEGIN) && (turn < max_turns)) {
			ret = serial_read_blocking(serial, &buf[0], 1, ebd_serial_timeout(serial, 1));
			if (ret < 0) {
		...
		while ((buf[message_length - 1] != MSG_FRAME_END)
			...
			ret = serial_read_blocking(serial, &buf[message_length], 1, ebd_serial_timeout(serial, 1));
			if (ret < 0) {
				sr_err("Error %d reading byte.", ret);
				return ret;
			} else if (ret == 1) {
 ...
 */
SR_PRIV int ebd_receive_data(int fd, int revents, const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
