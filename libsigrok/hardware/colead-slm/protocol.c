 ...
 * default).
 */
static int colead_slm_serial_timeout(struct sr_serial_dev_inst *serial,
		int bytes)
{
	uint64_t baudrate = 0;
	const char *sc;
 ...
 */
SR_PRIV int colead_slm_receive_data(int fd, int revents,
		const struct sr_dev_inst *sdi)
{
	...
	if (devc->state == IDLE) {
		...
		delay_ms = colead_slm_serial_timeout(serial, 1);
