 ...
 * functions. The greatfet driver uses alloc/submit_many/flush/free. This
 * minimal local implementation covers those four entry points. The struct
 * is opaque to api.c (which only holds a pointer in devc->acquisition.
 * feed_queue) so the definition here is self-contained. See
 * asix-omega-rtm-cli/protocol.c and kingst-la2016/protocol.c for similar
...
/*
 * Submit multiple samples at once. Each sample is unit_size bytes. The
 * data buffer must contain count * unit_size bytes. Flushes the internal
 * buffer automatically when it fills up.
 */
static int feed_queue_logic_submit_many(struct feed_queue_logic *q,
		const uint8_t *data, size_t count)
{
	...
	while (count > 0) {
		...
		memcpy(wrptr, data, to_copy * q->unit_size);
		data += to_copy * q->unit_size;
		q->count_samples += to_copy;
		count -= to_copy;
	}
...
/* ===========================================================================
 * Local sr_parse_probe_names / sr_free_probe_names implementation
 *
 * PXView's compat layer declares sr_parse_probe_names() with a different
 * (incompatible) signature and a stub body. The greatfet driver uses the
 * canonical sigrok signature: parse a comma-separated user string into a
 * NULL-terminated char** array of probe names, falling back to defaults
 * when the user string is NULL or shorter than the default list.
 * ========================================================================== */
		...
		size_t default_count, size_t *count)
{
	...
	if (!defaults || !max_count)
		return NULL;

	if (default_count > max_count)
		default_count = max_count;

	names = g_malloc0((max_count + 1) * sizeof(char *));
	...
	if (str && *str) {
		...
		for (l = tokens; l; l = l->next) {
			if (n >= max_count)
				break;
			names[n++] = g_strdup((const char *)l->data);
		}
		g_slist_free(tokens);
		g_free(tmp);

		while (n < default_count) {
			names[n] = g_strdup(defaults[n]);
			n++;
		}
	} else {
		for (n = 0; n < default_count; n++)
			names[n] = g_strdup(defaults[n]);
	}
