...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	usb_source_add(ctx, 50,
		greatfet_receive_data, sdi);
...
/*
 * Silence "unused function" warnings for dev_clear/clear_helper which
 * are not referenced by the PXView driver_info struct (PXView uses
 * dev_destroy instead of dev_clear). They remain here for reference and
 * would be used if a future PXView build adds dev_clear support.
 */
static void (*greatfet_unused_clear_helper)(struct dev_context *)
		= clear_helper;
static int (*greatfet_unused_dev_clear)(const struct sr_dev_driver *)
		= dev_clear;

/* ===========================================================================
 * PXView compat wrapper layer
 *
 * PXView's sr_dev_driver callbacks have different signatures from
 * standard sigrok's: init/cleanup/scan lack the driver parameter,
 * config_get/set/list use int keys and an extra channel parameter, and
 * acquisition_start/stop take a cb_data argument. The wrappers below
 * adapt the standard sigrok implementations to PXView's calling
 * convention. See microchip-pickit2/api.c for the same pattern.
 * ========================================================================== */

/* Static driver pointer for wrapper functions */
static struct sr_dev_driver *greatfet_drv_ptr;

/* Forward declaration - defined at end of file */
extern SR_PRIV struct sr_dev_driver greatfet_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int greatfet_compat_init(struct sr_context *sr_ctx)
{
	greatfet_drv_ptr = &greatfet_driver_info;
	return std_init(greatfet_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int greatfet_compat_cleanup(void)
{
	return std_cleanup(greatfet_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *greatfet_compat_scan(GSList *options)
{
	return scan(greatfet_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int greatfet_compat_config_get(int id, GVariant **data,
...
static int greatfet_compat_config_set(int id, GVariant *data,
...
static int greatfet_compat_config_list(int info_id, GVariant **data,
		const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int greatfet_compat_acquisition_start(struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
...
static int greatfet_compat_acquisition_stop(const struct sr_dev_inst *sdi,
		void *cb_data)
{
	(void)cb_data;
...
SR_PRIV struct sr_dev_driver greatfet_driver_info = {
	.name = "greatfet",
	.longname = "Great Scott Gadgets GreatFET One",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = greatfet_compat_init,
	.cleanup = greatfet_compat_cleanup,
	.scan = greatfet_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = greatfet_compat_config_get,
	.config_set = greatfet_compat_config_set,
	.config_list = greatfet_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = greatfet_compat_acquisition_start,
	.dev_acquisition_stop = greatfet_compat_acquisition_stop,
	.priv = NULL,
};
