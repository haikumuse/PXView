	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_DATA_SOURCE:
		if ((idx = local_std_str_idx(data, data_sources, ARRAY_SIZE(data_sources))) < 0)
			return SR_ERR_ARG;
...
static struct sr_dev_driver *agilent_dmm_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver agdmm_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int agilent_dmm_compat_init(struct sr_context *sr_ctx)
{
	agilent_dmm_drv_ptr = &agdmm_driver_info;
	return std_init(agilent_dmm_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int agilent_dmm_compat_cleanup(void)
{
	return std_cleanup(agilent_dmm_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *agilent_dmm_compat_scan(GSList *options)
{
	return scan(agilent_dmm_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int agilent_dmm_compat_config_get(int id, GVariant **data,
...
static int agilent_dmm_compat_config_set(int id, GVariant *data,
...
static int agilent_dmm_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int agilent_dmm_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
static int agilent_dmm_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver agdmm_driver_info = {
	.name = "agilent-dmm",
	.longname = "Agilent U12xx series DMMs",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = agilent_dmm_compat_init,
	.cleanup = agilent_dmm_compat_cleanup,
	.scan = agilent_dmm_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = agilent_dmm_compat_config_get,
	.config_set = agilent_dmm_compat_config_set,
	.config_list = agilent_dmm_compat_config_list,
	.dev_open = std_serial_dev_open,
	.dev_close = std_serial_dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = agilent_dmm_compat_acquisition_start,
	.dev_acquisition_stop = agilent_dmm_compat_acquisition_stop,
	.priv = NULL,
};