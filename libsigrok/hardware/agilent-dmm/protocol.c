...
/*
 * Local implementations of the sr_sw_limits helpers (PXView does not
 * provide them). Modelled on standard sigrok's semantics.
 */
...
SR_PRIV int agdmm_receive_data(int fd, int revents, const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
...
static int recv_fetc(const struct sr_dev_inst *sdi, GMatchInfo *match)
{
	...
	if (!strcmp(s, "-9.90000000E+37") || !strcmp(s, "+9.90000000E+37")) {
		...
		if (local_sr_atof_ascii(mstr, &fvalue) != SR_OK) {
			g_free(mstr);
	...
	if (devc->cur_unit[i] == SR_UNIT_DECIBEL_MW ||
		...
		if (mstr && local_sr_atoi(mstr, &exp) == SR_OK) {
...
static int recv_conf_u124x_5x(const struct sr_dev_inst *sdi, GMatchInfo *match)
{
	...
	if (rstr && local_sr_atoi(rstr, &resolution) == SR_OK) {
                    ...
                    unsigned int num_functions)
{
	...
	mstr = g_match_info_fetch(match, 2);
	if (local_sr_atoi(mstr, (int*)&function) != SR_OK || function >= num_functions) {
		g_free(mstr);
	...
	mstr = g_match_info_fetch(match, 3);
	if (local_sr_atoi(mstr, &value) != SR_OK) {
		g_free(mstr);
