 ...
 * that standard sigrok uses. Adapt handle_events accordingly: drop the
 * cb_data cast and take sdi as a const pointer.
 */
		...
		const struct sr_dev_inst *sdi)
{
	...
	if (revents & G_IO_IN) {
		siglent_sdl10x0_receive_data((struct sr_dev_inst *)sdi);
	} else {
		return FALSE;
	}
