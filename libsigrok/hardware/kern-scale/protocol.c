...
/*
 * KERN scale protocol parser and receive-data callback.
 *
 * Ported from standard sigrok's src/scale/kern.c (parser) and
 * src/hardware/kern-scale/protocol.c (receive path). The original driver
 * used a struct scale_info wrapper around sr_dev_driver plus per-variant
 * callbacks; since PXView's sr_dev_driver layout is fixed and only the
 * kern-ew-6200-2nm variant is registered here, the variant parameters
 * (vendor/model/conn/packet_size) are hardcoded in api.c and the parser
 * is invoked directly.
 */
...
/*---- KERN packet parser (ported from src/scale/kern.c) ---------------------*/
...
/*---- Receive path (ported from src/hardware/kern-scale/protocol.c) --------*/

/*
 * The KERN EW 6200-2NM uses 15-byte packets (or 14-byte, depending on user
 * configuration). Both variants are accepted transparently by the parser,
 * which derives the actual length from the position of the trailing CR/LF.
 */
#define KERN_PACKET_SIZE 15

static void handle_packet(const uint8_t *buf, const struct sr_dev_inst *sdi,
		void *info)
{
	float floatval;
...
static void handle_new_data(const struct sr_dev_inst *sdi, void *info)
{
	struct dev_context *devc;
	int len, offset;
	...
	offset = 0;
	while ((devc->buflen - offset) >= KERN_PACKET_SIZE) {
		if (sr_kern_packet_valid(devc->buf + offset)) {
			handle_packet(devc->buf + offset, sdi, info);
			offset += KERN_PACKET_SIZE;
		} else {
			offset++;
		}
	}
