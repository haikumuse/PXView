...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	uint64_t max_samplerate, baudrate;
	 ...
	 * speed / bitrate. Standard sigrok uses serial->comm_params.bit_rate
	 * which PXView's sr_serial_dev_inst does not expose, so parse the
	 * baudrate from the serialcomm string instead.
	 */
	baudrate = itech_it8500_parse_baudrate(serial);
	max_samplerate = baudrate * 15 / 9600;
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	ret = serial_source_add(serial, G_IO_IN,
			(int)(1000.0 / devc->sample_rate),
			itech_it8500_receive_data, sdi);
	if (ret == SR_OK) {
 ...
 * The original itech-it8500 driver only sends DF_END and removes the serial
 * source (it does not close the serial port here; that happens in dev_close).
 * Adapted to PXView's 1-arg serial_source_remove and 2-arg std_session_send_df_end.
 */
 ...
 * in the PXView compat layer. Inline the logic: iterate instances, call
 * dev_clear_callback on each devc to clear the mutex, then use
 * std_dev_clear_compat to free everything. Same pattern as the chronovu-la
 * compat driver.
 */
...
static struct sr_dev_driver *itech_it8500_drv_ptr;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int itech_it8500_compat_init(struct sr_context *sr_ctx)
{
	itech_it8500_drv_ptr = &itech_it8500_driver_info;
	return std_init(itech_it8500_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int itech_it8500_compat_cleanup(void)
{
	return std_cleanup(itech_it8500_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *itech_it8500_compat_scan(GSList *options)
{
	return scan(itech_it8500_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int itech_it8500_compat_config_get(int id, GVariant **data,
...
static int itech_it8500_compat_config_set(int id, GVariant *data,
...
static int itech_it8500_compat_config_list(int info_id, GVariant **data,
...
static int itech_it8500_compat_acquisition_start(struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
static int itech_it8500_compat_acquisition_stop(const struct sr_dev_inst *sdi,
	void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver itech_it8500_driver_info = {
	...
	.init = itech_it8500_compat_init,
	.cleanup = itech_it8500_compat_cleanup,
	.scan = itech_it8500_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = itech_it8500_compat_config_get,
	.config_set = itech_it8500_compat_config_set,
	.config_list = itech_it8500_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = itech_it8500_compat_acquisition_start,
	.dev_acquisition_stop = itech_it8500_compat_acquisition_stop,
	.priv = NULL,
};
