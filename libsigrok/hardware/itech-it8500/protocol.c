 ...
 * Returns 0 if parsing fails. PXView's sr_serial_dev_inst does not expose
 * a comm_params.bit_rate field like standard sigrok does, so derive the
		...
		struct itech_it8500_cmd_packet **response)
{
	...
	ret = serial_write_blocking(serial, cmd_buf, IT8500_PACKET_LEN,
			itech_it8500_serial_timeout(serial, IT8500_PACKET_LEN));
 ...
 * (PXView's sr_receive_data_callback_t expects this signature).
 */
