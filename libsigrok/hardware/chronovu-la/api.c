...
static struct sr_dev_driver *chronovu_la_drv_ptr;

/* Forward declaration - defined at end of file */
extern struct sr_dev_driver chronovu_la_driver_info;

/* Wrapper: PXView init(sr_ctx) -> standard init(driver, sr_ctx) */
static int chronovu_la_compat_init(struct sr_context *sr_ctx)
{
	chronovu_la_drv_ptr = &chronovu_la_driver_info;
	return std_init(chronovu_la_drv_ptr, sr_ctx);
}

/* Wrapper: PXView cleanup(void) -> standard cleanup(driver) */
static int chronovu_la_compat_cleanup(void)
{
	/* Call dev_clear to free per-device resources before cleanup */
	dev_clear(chronovu_la_drv_ptr);
	return std_cleanup(chronovu_la_drv_ptr);
}

/* Wrapper: PXView scan(options) -> standard scan(driver, options) */
static GSList *chronovu_la_compat_scan(GSList *options)
{
	return scan(chronovu_la_drv_ptr, options);
}

/* Wrapper: PXView config_get(id,data,sdi,ch,cg) -> standard(key,data,sdi,cg) */
static int chronovu_la_compat_config_get(int id, GVariant **data,
...
static int chronovu_la_compat_config_set(int id, GVariant *data,
...
static int chronovu_la_compat_config_list(int info_id, GVariant **data,
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
...
static int chronovu_la_compat_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
static int chronovu_la_compat_acquisition_stop(const struct sr_dev_inst *sdi, void *cb_data)
{
	(void)cb_data;
...
struct sr_dev_driver chronovu_la_driver_info = {
	.name = "chronovu-la",
	.longname = "ChronoVu LA8/LA16",
	.api_version = 1,
	.driver_type = DRIVER_TYPE_HARDWARE,
	.init = chronovu_la_compat_init,
	.cleanup = chronovu_la_compat_cleanup,
	.scan = chronovu_la_compat_scan,
	.dev_mode_list = compat_dev_mode_list_default,
	.config_get = chronovu_la_compat_config_get,
	.config_set = chronovu_la_compat_config_set,
	.config_list = chronovu_la_compat_config_list,
	.dev_open = dev_open,
	.dev_close = dev_close,
	.dev_destroy = compat_dev_destroy_default,
	.dev_status_get = compat_dev_status_get_default,
	.dev_acquisition_start = chronovu_la_compat_acquisition_start,
	.dev_acquisition_stop = chronovu_la_compat_acquisition_stop,
	.priv = NULL,
};