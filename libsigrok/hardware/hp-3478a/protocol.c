...
/*
 * PXView note: the parameter is const because this is invoked from the
 ...
 * directly (matching the hp-3457a / fluke-dmm compat driver pattern).
 * The devc->sr_digits value computed during status byte parsing is
 * informational only (PXView's analog struct has no digits field).
 */
static void acq_send_measurement(const struct sr_dev_inst *sdi)
{
	...
	/*
	 * Original used meaning.channels = sdi->channels via the new sigrok
	 * analog API. Map to the old flat struct's probes field and free it
	 * after sending the packet.
	 */
	analog.probes = g_slist_copy(sdi->channels);
	...
	analog.mq = devc->measurement_mq;
	analog.mqflags = devc->acquisition_mq_flags;
	analog.unit = devc->measurement_unit;
 ...
 *
 * sr_scpi_gpib_spoll() is not available in PXView's compat layer (no GPIB
 * transport). When it returns an error, skip the SRQ check and fall through
 * to direct reading, matching the approach used by other PXView SCPI DMM
 * drivers (e.g. hp-3457a) which read directly without spoll.
 */
SR_PRIV int hp_3478a_receive_data(int fd, int revents, const struct sr_dev_inst *sdi)
{
	 ...
	 *
	 * In PXView (no GPIB transport), spoll is unavailable; fall through to
	 * a direct read instead of removing the source.
	 */
