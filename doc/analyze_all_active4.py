...
for t in threads:
    ...
    if in_window_weight > 500:
        print(f"\n=======================================================")
        print(f"THREAD: {name} (TID: {t.get('tid')}) - Samples >= 6.7s: {in_window_weight}")
        print(f"=======================================================")
        
        print("TOP 5 EXCLUSIVE FUNCTIONS:")
        ...
        print("\nTOP 2 ACTIVE CALL STACKS:")
        sorted_paths = sorted(in_window_paths.items(), key=lambda x: x[1], reverse=True)
        for rank, (path, cnt) in enumerate(sorted_paths[:2]):
            pct = (cnt / in_window_weight) * 100
            print(f"  Rank {rank+1} (Weight: {cnt}, Percentage: {pct:.2f}%)")
            parts = path.split(" -> ")
